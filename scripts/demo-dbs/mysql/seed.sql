-- Kira Studio — MySQL seed data (idempotent)
-- Run: docker exec -i kira-mysql mysql -ukira -pkira kira < scripts/demo-dbs/mysql/seed.sql
--
-- A port of ../mariadb/seed.sql, not a copy: every row-generating SELECT below is the same shape,
-- substituting a `WITH RECURSIVE` numbers CTE for MariaDB's seq_1_to_N pseudo-table (P34 F13 —
-- MySQL has no SEQUENCE storage engine) and UUID() results into CHAR(36) columns instead of a
-- native UUID type (same substitution as mysql/init.sql).

SET NAMES utf8mb4;
-- Default is 1000 — well under the 80,000 order_items needs.
...
WITH RECURSIVE nums(seq) AS (
    SELECT 1
    UNION ALL
    SELECT seq + 1 FROM nums WHERE seq < 20000
)
SELECT
...
WITH RECURSIVE nums(seq) AS (
    SELECT 1
    UNION ALL
    SELECT seq + 1 FROM nums WHERE seq < 80000
)
SELECT
    ((seq - 1) % 20000) + 1,
    ((seq * 7919) % 20000) + 1,
