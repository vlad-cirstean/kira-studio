...
// P34 D7: one Adapter implementation for both MariaDB and MySQL, parameterized by a
// MysqlFamilyProfile (kind/serverLabel/applyEngineOptions — D9) plus a per-engine `caps` literal
// (D10: identical values today, stated per engine rather than shared, so a future divergence has
// somewhere honest to be said).
class MysqlFamilyAdapter implements Adapter {
  readonly kind: Adapter['kind'];
  readonly caps: Caps;
  ...
  constructor(
    ...
    private readonly profile: MysqlFamilyProfile,
    caps: Caps,
  ) {
  ...
  async connect(cfg: ResolvedConnectionConfig, ctx: OpCtx): Promise<ConnectInfo> {
    const connectionSet = new ConnectionSet(cfg, this.deps.log, this.profile.applyEngineOptions);
  ...
  async cancel(opId: string): Promise<boolean> {
    ...
    const options = buildConnectionOptions(this.cfg, {
      log: this.deps.log,
      applyEngineOptions: this.profile.applyEngineOptions,
    });
    ...
    try {
      ...
      this.deps.log(
        'warn',
        `${this.kind} cancel(${opId}) failed: ${err instanceof Error ? err.message : String(err)}`,
      );
      return false;
    } finally {
...
export function createMysqlFamilyAdapter(
  deps: AdapterDeps,
  profile: MysqlFamilyProfile,
  caps: Caps,
): Adapter {
  return new MysqlFamilyAdapter(deps, profile, caps);
}
