...
<template>
  ...
  <div class="search-toolbar p-toolbar" data-testid="keyvalue-search-toolbar" @keydown="onKeydown">
    ...
    <template v-else>
      ...
      <IconButton icon="chevron-up" :size="12" title="Previous match" data-testid="keyvalue-search-prev" @click="goPrev" />
      <IconButton icon="chevron-down" :size="12" title="Next match" data-testid="keyvalue-search-next" @click="goNext" />
      <div class="sep" />
      <span class="p-xs dim">in the {{ loadedRowCount.toLocaleString() }} loaded rows</span>
    </template>
    <IconButton icon="close" class="p-push" title="Close" data-testid="keyvalue-search-close" @click="close" />
  </div>
</template>
