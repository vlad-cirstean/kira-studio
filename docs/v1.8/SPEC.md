...
| **P86 Status-bar agent widget: keep-awake control (off / while an agent runs / always on)** | Clicking P85's running-agent-sessions widget opens a small control (a popover or menu — this phase's own planning pass picks the shape, matching other status-bar/toolbar popovers already in this app) with three keep-the-computer-awake options: off, on while at least one Claude Code session is running, and always on. Backend is a new small Go package behind an OS-abstracted interface (one acquire/release pair for a keep-awake assertion) so the code shape stays modular for a future Windows/Linux implementation — only a macOS implementation ships this phase, every other OS's implementation is a documented no-op. **Decided, not left open: spawn/kill `caffeinate -di` as a child process, never a direct `IOPMAssertionCreateWithName`/`IOPMAssertionRelease` cgo call.** Wails itself has no power-management API of its own — there is nothing framework-level to reach for — but `caffeinate` is Apple's own supported CLI wrapping that identical IOKit API, which makes it the "existing, well-maintained tool" this repo's own "reach for existing tooling" rule asks for, not a library to hand-roll around. The direct-API alternative means writing and maintaining cgo/Objective-C glue with no realistic way to unit-test it in this sandbox (no display session, no observable effect short of reading `pmset` output anyway) — explicitly rejected on exactly that "no hand-rolled, untestable native glue" basis, not a coin flip between two live options. The subprocess's lifetime tracks the "while an agent runs" mode's own start/stop transitions (start one when the first Claude Code session begins, kill it when the last one ends) — this phase's own planning pass designs that lifecycle concretely, and states how it's tested (spawn, assert via `pmset -g assertions` that the entry appears, kill, assert it's gone — a real integration test, not a mock) | Needs P85's widget landed first — this phase's whole surface is that widget's click target; independent of P84's script-launcher content otherwise |
