...
**Sixteen feature rows plus the two standing code-review rounds and the closing docs row.** Every
...
widget, then a full dropdown redesign, then a new context-menu action). P84 (Cmd/Ctrl+click
navigation) is appended last among the feature rows — found mid-chapter, during a code-intelligence
discussion unrelated to any phase's own planning pass, the same "discovered live, not by a phase's
own plan" shape v1.6's P67b-P67d additions used, so it's numbered as a new row rather than folded
into an existing one; it has no dependency on P71-P83 and could in principle land anywhere, placed
last only because it was added last. P85 and P86 — extending code-graph matching to resolve Go
inheritance, then wiring find-references/go-to-implementation into the editor — were added in the
same discussion, for the same reason: found live, not by any phase's own planning pass. P85 before
P86 because P86's own value for Go depends on it: `internal/codegraph/implementations.go` returns
nothing for Go today (`ImplementationsOf`'s own comment: "no receiver stored, no method set to
compare" — a captured-data gap, not a resolver bug, per `internal/codeparse/queries/go/tags.scm`
capturing a `method_declaration`'s name only), so wiring a Go-to-implementation UI before that's
fixed would ship a command that silently does nothing for the app's own primary backend language.
P87/P88 (the two code-review rounds) and P89 (main-docs update) close the chapter exactly as
`CLAUDE.md`'s standing process and v1.6/v1.7's own closing rows require.
...
| **P84 Editor: Cmd/Ctrl+click does not perform go-to-definition navigation** | The Monaco `DefinitionProvider` registration (`apps/kira-studio/frontend/src/views/repo/navigation.ts`, `ensureNavigationRegistered()`) already backs F12/hover, per this chapter's own code-intelligence research pass, but Cmd/Ctrl+click does not navigate. This phase's own planning pass root-causes it before fixing — candidates to rule in/out: the modifier-click handler never reaching `registerDefinitionProvider`'s codepath at all (a separate click-binding gap, not a resolver bug), a platform modifier-key mismatch (Cmd on macOS vs Ctrl elsewhere) in whatever binds the click, an editor option (`multiCursorModifier`/`gotoLocation` or similar) shadowing the default binding, or the provider itself throwing/returning nothing silently on click-triggered requests specifically (vs. the explicit F12 command path). Fix only once the actual cause is confirmed, not by guessing | Found live during this chapter's own code-intelligence discussion, unrelated to any other row's planning pass — no dependency on P71-P83, placed last among the feature rows only because it was added last |
| **P85 Extend code-graph matching to resolve Go inheritance** | `internal/codegraph` resolution is name/scope-proximity only, by explicit design (`resolve.go` ranks same-named candidates by same-file → same-directory → repo-wide, never receiver-type or method-set); the hover UI itself already labels every result "_Name-resolved, not type-resolved._" TS/Java already go further via `extends_clause`/`implements_clause`/`superclass` captures feeding `containmentImplementationsOf`, so this row is Go-specific. Root cause, confirmed by reading `internal/codeparse/queries/go/tags.scm`: `method_declaration` captures a method's name only, never its receiver type, so no type→method-set index can exist. This phase's own planning pass adds a receiver-type capture to that query, extends `go/m1c_fields.scm`'s existing struct-field capture to the embedded-field case it already documents skipping ("An embedded field has no `name` field and is not matched"), builds the resulting type→method-set index, and updates the Go resolver path to check method-set/embedding membership before falling back to name-proximity — closing `ImplementationsOf`'s Go path, which returns nothing today | Found live during this chapter's own code-intelligence discussion; placed before P86 since P86's Go-to-implementation UI depends on this data existing to be useful |
| **P86 Wire find-references and go-to-implementation into the editor** | `apps/kira-studio/frontend/src/views/repo/navigation.ts` registers `DefinitionProvider`/`HoverProvider` but never `registerReferenceProvider`/`registerImplementationProvider`, even though Monaco's own stock peek-references/peek-implementation UI is already fully bundled and registered in this app (`register.all.js` pulls in `ReferencesController`, `GoToImplementationAction`, `PeekReferencesAction` — confirmed present, not stripped) and the backend already returns lists (`internal/codegraph`'s `ReferencesTo`/`ImplementationsOf`). The blocker this phase's own planning pass must solve isn't UI absence: `Refs.Sites` carries no per-occurrence confidence/rule (only a repo-wide `Unattributed` aggregate), and a bare `Location[]` handed to Monaco's stock peek list can't distinguish a confident hit from an ambiguous repo-wide name match, or "unsupported for this language" from "genuinely none found" — the same distinction the hover provider already renders in its markdown for go-to-definition. Design and implement the equivalent surfacing for references/implementations (extending what's returned, a label/decoration in the peek list, or another mechanism the plan justifies) before wiring the providers, not after | Right after P85, so Go's own implementation results are real by the time this phase wires the command that surfaces them |
| **P87 Code review, round 1** | Three parallel Opus subagents, one per dimension — architecture/security, functional correctness, performance/resource efficiency — findings-only, per `CLAUDE.md`'s own process, scoped to this chapter's own diff (P71-P86). One sequential Sonnet subagent fixes every finding judged real | After P71-P86 land: a review needs a finished tree |
| **P88 Code review, round 2** | The same three-dimension cycle, run again in full — against the tree P87's fixes leave, re-read fresh rather than trusting P87's summary — per `CLAUDE.md`'s "repeat the whole loop" rule. A round finding nothing real says so rather than manufacturing a finding | After P87's fixes land |
| **P89 Update main docs** | Brings `README.md`, `docs/ARCHITECTURE.md`, `docs/DEV_ENVIRONMENT.md`, `CLAUDE.md` current for this chapter's changes — API incognito mode, the git module's rendering/PR/diff/review/settings/blame/worktree changes, the Cmd/Ctrl+click navigation fix, and the extended code-graph matching/navigation wiring — the same "read the current tree, don't trust prose" bar v1.6's P70 and v1.7's M8 used | Last of all: needs both review rounds' fixes landed first |
