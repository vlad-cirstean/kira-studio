...
additions requested directly, not found by any prior phase's own work. P84 (Claude Code and
user-configured custom scripts, launched from P83's own tab-strip button), P85 (opt-in Claude Code
lifecycle hooks plus a running-agent-sessions status-bar widget) and P86 (that widget's own
keep-awake control) extend the chapter again, each strictly needing the row before it landed first —
together they turn the left panel and tab strip P82/P83 build into the start of an agentic
development environment, not a stopping point after P83.
...
| **P84 Tab-strip "+" button: launch Claude Code or a configured custom script in a terminal** | Extends P83's tab-strip dropdown (currently one entry, "Terminal") with two more kinds: "Claude Code" (launches the `claude` CLI in a new terminal tab, over the same PTY/worktree-cwd machinery P83 built) and one entry per user-configured custom script. A new "Manage scripts…" entry in the same dropdown opens a config surface (this phase's own planning pass places it — a small dialog, or a new `SettingsDialog.vue` section, whichever fits the existing shape) for adding/editing/removing named scripts (a shell command, a working-directory default, maybe an icon/color) — never predefined by this phase, entirely user-configured, starting empty. Every launched entry opens as the same terminal tab kind P83 built: full PTY output visible, closeable, reopenable — this phase adds launch targets to that mechanism, not a second one | Needs P83's terminal tab kind and PTY plumbing landed first; the dropdown entries and script-config surface are new UI surface this phase alone owns |
| **P85 Claude Code lifecycle hooks, opt-in and user-configured, plus a running-agent-sessions status-bar widget** | Wires Claude Code's own hook system (`SessionStart`/`SessionEnd`/`PreToolUse`/`PostToolUse`/`Notification`/`Stop` — the same mechanism this very repo's `.claude/settings.json` already uses for its own `PostCompact` hook) into every "Claude Code" terminal tab P84 launches — not auto-configured: a user must opt in and set it up from Settings the first time (this phase's own planning pass designs that first-run flow), never silently written into a project's `.claude/settings.json` without the user seeing and confirming it. Separately: a new `StatusBar.vue` widget — same pattern P76's blame item and P78's nav-status item already establish, icon plus a number, absent rather than a zero reading when nothing is running, per that file's own "absent, not a zero reading" convention (`StatusBar.vue:72`-`74`'s comment) — tracks how many Claude Code sessions (P84's launched terminals) are currently running, counted across every open workspace, not per-tab | Needs P84's Claude Code launch entry landed first — there is nothing to hook or count before a session can be launched from the app; the widget is P86's own click target, so P86 follows this row |
| **P86 Status-bar agent widget: keep-awake control (off / while an agent runs / always on)** | Clicking P85's running-agent-sessions widget opens a small control (a popover or menu — this phase's own planning pass picks the shape, matching other status-bar/toolbar popovers already in this app) with three keep-the-computer-awake options: off, on while at least one Claude Code session is running, and always on. Backend is a new small Go package behind an OS-abstracted interface (one acquire/release pair for a keep-awake assertion) so the code shape stays modular for a future Windows/Linux implementation — only a macOS implementation ships this phase, every other OS's implementation is a documented no-op. **Decided, not left open: spawn/kill `caffeinate -i -s` as a child process, never a direct `IOPMAssertionCreateWithName`/`IOPMAssertionRelease` cgo call.** Wails itself has no power-management API of its own — there is nothing framework-level to reach for — but `caffeinate` is Apple's own supported CLI wrapping that identical IOKit API, which makes it the "existing, well-maintained tool" this repo's own "reach for existing tooling" rule asks for, not a library to hand-roll around. The direct-API alternative means writing and maintaining cgo/Objective-C glue with no realistic way to unit-test it in this sandbox (no display session, no observable effect short of reading `pmset` output anyway) — explicitly rejected on exactly that "no hand-rolled, untestable native glue" basis, not a coin flip between two live options. **Confirmed against real prior art, not guessed**: Orca (`stablyai/orca`), an agent development environment solving this identical problem, ships exactly this — `caffeinate -i -s` (prevent idle + system sleep, display still allowed to dim/lock) started when an eligible agent begins and killed when the last one ends, the same trigger shape as this row's "while an agent runs" mode, plus one detail this plan folds in: **the assertion is re-armed on a power-resume event**, since a live `caffeinate` process's effect doesn't reliably survive a sleep/wake cycle. The subprocess's lifetime tracks the "while an agent runs" mode's own start/stop transitions (start one when the first Claude Code session begins, kill it when the last one ends, re-spawn on wake) — this phase's own planning pass designs that lifecycle concretely, and states how it's tested (spawn, assert via `pmset -g assertions` that the entry appears, kill, assert it's gone — a real integration test, not a mock). Deliberately **not** folded in from that same prior art: a separate "keep display awake" sub-toggle (`-d`, guarding SSH-agent/password-manager screen-lock timeouts) — a real, well-motivated feature, but a fourth option beyond what was asked for here; worth a future phase, not silently added to this one | Needs P85's widget landed first — this phase's whole surface is that widget's click target; independent of P84's script-launcher content otherwise |
