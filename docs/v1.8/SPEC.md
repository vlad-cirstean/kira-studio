...
| **P86 Status-bar agent widget: keep-awake control (off / while an agent runs / always on)** | Clicking P85's running-agent-sessions widget opens a small control (a popover or menu — this phase's own planning pass picks the shape, matching other status-bar/toolbar popovers already in this app) with three keep-the-computer-awake options: off, on while at least one Claude Code session is running, and always on. Backend is a new small Go package behind an OS-abstracted interface (one acquire/release pair for a keep-awake assertion) so the code shape stays modular for a future Windows/Linux implementation — only a macOS implementation ships this phase, every other OS's implementation is a documented no-op. Researched, not guessed: two real macOS mechanisms exist — spawning and killing a `caffeinate -di` child process (matches this repo's own "reach for existing tooling" rule, but ties the assertion's lifetime to a subprocess the "while an agent runs" mode must start/stop as sessions come and go), and calling `IOPMAssertionCreateWithName`/`IOPMAssertionRelease` directly (the same CoreFoundation/IOKit API `caffeinate` itself wraps, and the mechanism a dedicated app like KeepingYouAwake uses directly, acquired/released in-process with no subprocess to manage) — this phase's own planning pass states which it picks and why, rather than leaving both live | Needs P85's widget landed first — this phase's whole surface is that widget's click target; independent of P84's script-launcher content otherwise |
