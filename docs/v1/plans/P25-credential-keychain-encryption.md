...
systemd (CLAUDE.md's Docker section), therefore no `gnome-keyring` or `kwallet` daemon, therefore
...
CLAUDE.md                        MOD  short note: running the UI suite on Linux vs macOS
```
...
3. **Should `bun run dev` on Linux imply the fallback?** As written, a contributor on Linux has to
   export `KIRA_INSECURE_SECRETS=1` themselves (CLAUDE.md will say so); only the test fixture sets
