...
- **SQLite access is later layered with Drizzle ORM.** P0 opens `node:sqlite` directly through
  `db.ts` (D2/D3 below); P1 Step 13 wraps it in `drizzle-orm/sqlite-proxy` and moves table access
  onto Drizzle schema definitions that mirror the migration files here. See
  `docs/plans/P1-connections-and-tree.md` Step 13.
...
| D13 | **No electron-builder in P0.** | §10 puts unsigned packaging in P12. Adding it now means maintaining a config nothing exercises. |
...
- **No electron-builder / packaging / signing.** P12 (D13).
