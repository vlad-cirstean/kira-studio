...
- **Comments per CLAUDE.md** — only where the code cannot say it for itself. Three comments in the
...
- **Green after every commit:** `bun run lint`, `bun run typecheck`, `bun run build`, plus the four
  ...
  `connections` — CLAUDE.md's Electron-binary note). `bun run test:db` is untouched by this phase
