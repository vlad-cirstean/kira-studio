...
> (2026-08-28), and `docs/v1/plans/P51-spike-report-part{1,2,3}.md` are the report §6 asks for, so
> far — the subset answerable from a Linux sandbox with no macOS machine and with
> `wails.io`/`v3.wails.io` still egress-blocked. Part 1 covers §3.1, §3.2, §3.3, §3.6 and §3.10 with a
> real installed toolchain, a real generated Wails v3 project, and two measured Go↔Node prototypes.
> Part 2 reads the Wails v3 Go source directly and turns §3.2's bulk-data bridge from a hypothesis
> into a concrete design: a named `Stream()` carrying raw bytes for bulk pages, alongside the default
> JSON `Call` binding for everything else. Part 3 got a live `wails3 dev` server running and reachable
> (the earlier "sandbox can't keep a background process alive" read in part 2 turned out to be simply
> not waiting long enough — the real first build takes ~60s) and found something new: on Linux,
