...
- **Green after every commit:** `bun run lint`, all three `typecheck:*` splits, `bun run build`,
  ...
  Claude Code's Linux web container** (CLAUDE.md: the blocked `production.cloudfront.docker.com`
...
Full list in §5. **These cannot be executed in this sandbox** (CLAUDE.md's Docker CDN block); the
...
container. Per CLAUDE.md, Claude Code's Linux web containers cannot pull images
