...
- **Nothing the tree stops showing may become unreachable.** Every affordance a column tree row
  ...
  coverage "no longer applicable" would be exactly the half-implementation CLAUDE.md rules out.
...
- Comments per CLAUDE.md: only where the code cannot say it for itself. Every `D` below that encodes
