...
| **P49 Vue/Node ecosystem library spike** | Not a SPEC §10 deliverable — a post-v1 spike, gated per-library by the same evidence-based go/no-go discipline P47 used for its own decision (D14), rather than adopting or discarding anything by default. Two parts: (1) revisit P47's own "go" verdict now that it's had time to prove out only on the SQL grid — decide whether the other views' shared `VirtualList.vue` primitive (document, keyvalue, stream, console results) should also move to `@tanstack/vue-virtual`, with outright removal of the existing adoption back on the table if a fresh look finds it doesn't hold up even there; (2) broaden into a general survey of well-known libraries in the Vue ecosystem (TanStack Query, TanStack Form, Pinia, VueUse, and others surfaced during the spike itself, not limited to those named up front) and their Node.js backend equivalents, each judged against this codebase's actual needs. The deliverable is a written per-library analysis — why it would help or why it wouldn't — not a mandate to add any of them | Direct follow-up to P47, whose go/no-go only ever covered the SQL grid in isolation; the same rigor needs to run both backward (does that existing adoption still hold up) and outward (would it, or anything else commonly used in Vue/Node projects, help the views and backend code that don't yet use it) before treating P47's decision as the final word |
| **P50 Console/key-value/stream virtualization fixes and a shared overlay-positioning function** | Not a SPEC §10 deliverable — a direct implementation follow-up to three of P49's own findings, greenlit rather than left as "revisit later": (1) `ConsoleResultGrid.vue` renders every column of every visible row with no row cap on the query console's result set (P49 F9), unlike the SQL grid, which has windowed both axes since P29/P47 — give the console the same column axis, reusing the already-extracted, already-unit-tested `columnRangeExtractor`; (2) `KeyValueView.vue` and `StreamView.vue` render every loaded row directly with no virtualization at all, at page sizes up to 10 000 rows (P49 F7) — mount the app's existing `VirtualList.vue` primitive, the same one six other call sites already use; (3) `PopoverPanel.vue`, `AppTooltip.vue` and `ErrorPopover.vue` each hand-roll their own flip/clamp overlay-positioning arithmetic, already disagreeing with each other and already the cause of one shipped bug (P49 F16) — extract one shared positioning function rather than adopting `@floating-ui/vue`, the "no dependency" alternative P49 D12 itself named | The user's own decision after reviewing P49's spike results: fix the two virtualization defects P49 found and named as its most defensible findings (D4/D5), and address the overlay-positioning duplication P49 flagged (D12) now rather than waiting for a third bug, but as a shared function rather than a new dependency, since P49 itself never recommended the dependency for this — only the consolidation |
