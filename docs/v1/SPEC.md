...
| **P51 NeutralinoJS migration spike** | Not a SPEC §10 deliverable — a user-directed, deliberately scoped-down spike: on a branch cut from this point, get the app's existing built frontend rendering inside a NeutralinoJS window (a walking-skeleton launch), and record what that took and what it didn't reach. Explicitly **out of scope for this phase**: OS keychain/credential-encryption integration (the `safeStorage` equivalent, P25), the native DB adapters and their engine subprocess, the bulk-data `MessageChannelMain` transport, packaging, and an E2E harness — this phase is only about whether and how a Neutralino-hosted window can come up at all, not a full migration | Electron's baseline memory/idle footprint is a known, recorded cost (P20's own Electrobun spike; P46; `memory.spec.ts`'s removal in P50 for being permanently over budget on process overhead alone). P20 found Electrobun blocked on this sandbox's network and, once unblocked on another machine, blocked again on no WKWebView E2E driver and a bridge with no port-transfer primitive; a separate unmerged spike branch pivoted instead to Wails v3 + a Go native shell around a Node engine child process, with real progress there. NeutralinoJS is a third, much smaller candidate (a thin native-binary shell with no bundled Node.js runtime at all in its own process) worth its own narrowly-scoped, throwaway look now, before deciding whether it's worth the larger investment the full app would need — queued the same way P20 was: comparative, not a one-way door. Implemented per `docs/v1/plans/P51-neutralino-migration-spike.md` — unlike P20, the verdict was yes: the app's unmodified `out/renderer/` bundle renders the full workbench inside a real NeutralinoJS/WebKitGTK window with a ~30-line `window.kira` stub and zero console errors, and `neu build`'s packaged binary reproduces the dev-run output byte-for-byte. Five files committed (`neutralino/package.json`, `package-lock.json`, `neutralino.config.json`, `kira-stub.js`, `scripts/build-neutralino-shell.sh`); `src/`, `tests/`, the Electron config and the root `package.json`/`bun.lock` are untouched, and the real Electron app was re-verified booting on this same branch afterward. Bulk-data transport, the engine subprocess, keychain/credential storage, packaging and an E2E harness were deliberately left out and are named concretely in the plan's §4, along with the macOS-14-floor and no-CDP-driver findings carried over from P20 (§8) |
| **P52 NeutralinoJS engine and IPC bridge (all adapters)** | Not a SPEC §10 deliverable — the real-migration follow-up to P51's walking skeleton, user-directed: replace `src/main`'s Electron-specific IPC/window/lifecycle code and P51's `window.kira` stub with a real Neutralino-hosted bridge, bringing every existing adapter up at once — the adapters themselves are unmodified, transport-agnostic code (P51 reality #1/#7) and need no per-adapter work once the bridge exists. Architecture, decided with the user before this row was written: one Node process ("app process") registers as the sole Neutralino extension and owns the `nlPort`/`nlToken` handshake; that process spawns the DB engine as an ordinary Node child via `child_process.fork()` (no Neutralino involvement in that hop — the same primitive `utilityProcess.fork` already wraps today); traffic between the app process and the page rides Neutralino's `extensions.dispatch`/`app.broadcast` mechanism; bulk query-result pages are sent **chunked** (row batches, not one giant payload) to avoid blocking the app process's event loop and to avoid head-of-line blocking control-channel messages queued behind a large payload on the same relay. Two pieces of this have no existing precedent to port and are verified for real before the rest is built on top of them: the exact `extensions` wire protocol (only a client-library round trip was verified in P51 §0.7, not an extension process's own handshake/broadcast/dispatch), and the chunked relay itself (nothing chunks a result stream across `app.broadcast` today, in this codebase or in Neutralino's own examples). Explicitly out of scope, because each is a subsystem with no existing code to adapt rather than a staging choice: OS keychain/credential storage, packaging, and an E2E harness | Direct continuation of P51: the walking skeleton proved the frontend renders; this phase proves the thing P51 named as its biggest deferred question — whether an engine process can actually reach the frontend with real data under Neutralino's no-port-transfer constraint — is buildable and chunked, for the whole app at once, since the adapters ride along for free once the bridge exists. Keychain, packaging and an E2E harness stay separate only because they are genuinely new subsystems with nothing to port, not because of adapter-by-adapter risk-staging |
