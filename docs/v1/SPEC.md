...
| **P51 Tauri + Node sidecar migration spike** | Replace P51's former Electrobun slot with a different runtime candidate: investigate migrating the app off Electron onto **Tauri**, with a **Node.js sidecar** process. The architectural direction is fixed by the user, not open for the spike to decide: the Tauri (Rust) `src-tauri` core stays as barebones as possible — native window, native menu, app lifecycle, and spawning/supervising the sidecar — while virtually everything in `src/main` (IPC handler logic, the SQLite storage layer, settings/layout/tabs/connections/saved-query persistence, credential encryption, the op log) plus all of `src/engine` (adapters, cache, scheduler) moves into the Node sidecar, collapsing today's `src/main`-owns-IPC / `utilityProcess`-hosts-the-engine split. The motivation is unchanged from P20's: `docs/PERF.md` §2.2's ~620MB of Chromium/Electron baseline overhead against a 350MB budget, which no app-level lever can reach (`tests/e2e/memory.spec.ts` was removed rather than kept red, commit `d23121e`). Electrobun's two dealbreakers do not transfer 1:1 — Tauri has a first-party WebDriver story (`tauri-driver`) rather than none, and first-class sidecar support via `tauri.conf.json`'s `bundle.externalBin` / `tauri-plugin-shell`, which may let bulk data bypass the core entirely instead of regressing — but both must be independently re-established for Tauri, not assumed solved. The spike's deliverable is a written report: the renderer↔sidecar transport design (route through Tauri's `invoke` bridge vs. a direct loopback connection to the sidecar), the bulk-data design with no `MessagePort` primitive, the sidecar packaging/spawning story, sidecar lifecycle and supervision, the `safeStorage`-replacement seam, the E2E-testing story, the two-binary macOS packaging/signing story, and a completed per-module disposition of `src/main`/`src/engine` — plus a go/no-go recommendation | Not yet planned for implementation — **plan only**, see `docs/v1/plans/P51-tauri-node-sidecar-spike.md`. That document is this phase's entire output so far: **no Tauri or Rust toolchain install, no `src-tauri` scaffold, no code change, no dependency addition and no measurement have been performed**, and every claim it makes about Tauri itself (its IPC model, the `externalBin` sidecar mechanism, `tauri-driver`'s platform support, its credential-storage plugins) is flagged as best-current-knowledge for a future spike stage to confirm directly rather than asserted as fact. It flags two things up front as the cheapest possible stops: `tauri-driver` is believed **not** to support macOS (Apple ships no WebDriver server for WKWebView), which would be the same class of dealbreaker P20 hit for this macOS-only app and should be checked before any other spike work; and OS-keychain access has no Node-side equivalent, so P25's `safeStorage`-based credential encryption needs a small privileged Rust surface — the one place the barebones-core direction has to bend. This row supersedes the former "P51 Electrobun migration spike, reopened"; P20's own row and its out-of-scope verdict are unchanged. Requires the user's explicit sign-off on the finished spike report before any implementation begins |
