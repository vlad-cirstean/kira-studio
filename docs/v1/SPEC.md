...
| **P45 Docs cleanup** | Not a SPEC §10 deliverable — a user-directed, post-v1 phase: SPEC.md and its `plans/` are retired as v1 history rather than a living spec; whatever in them is still true and durable moves into `ARCHITECTURE.md`, which `CLAUDE.md` points to and which itself points out to the general docs (`PACKAGING.md`, `PERF.md`); the `docs/` folder structure is reconsidered to match | The user's own framing: spec and its plans are now "an element of the past of v1" | Implemented per `docs/v1/plans/P45-docs-cleanup.md`, twelve commits (`395423e`…`59e1a77`) plus this row. The mandate's premise was half-true: ARCHITECTURE.md/PACKAGING.md/PERF.md already existed (under `docs/v1/`), and CLAUDE.md already pointed at ARCHITECTURE.md six times; what was actually still true was that ARCHITECTURE.md deferred to SPEC.md for four live facts and named it authoritative in its own second paragraph (D62/D60). `docs/v1/` disposition (D57/D58): SPEC.md and `plans/` do **not** move — 56 in-repo citations name `docs/v1/plans/` by path, six of them live `src/`/`tests/` comments, and moving them would dangle every one and force retro-editing frozen history. The restructure goes the other way: `ARCHITECTURE.md`/`PACKAGING.md`/`PERF.md` move up to `docs/`, `design/` to `docs/design/`, so `docs/*.md` is living and `docs/v1/` (now holding exactly `SPEC.md`, `plans/` and a new archive-banner `README.md`) is the v1 record — repairing four references left dangling by an earlier `f660ffd` move along the way (commit 1). ARCHITECTURE.md absorbed (D62) the process model, storage, caching, the §5.1 per-engine mapping table, the stack, §2's invariants (with a pointer to `docs/PERF.md` §1 for the budget numbers rather than a duplicate table) and a testing strategy corrected to name all four `tests/` directories and D66's rule (`tests/unit/` needs nothing and finishes in about a second, `tests/db/`/`tests/electron-db/` need a real external resource) — each replacing a stub or filling a gap that had never existed. It deliberately did not absorb §8's 604 UI lines verbatim (D63): a new ~55-line UI architecture section distills view-kind-by-page-kind selection, the tab-identity rule, the session-restore reconnect gate, the staged-vs-immediate write model, the cell editor as a per-view mounted panel, the Browse panel's one-level model, and the shared find/chunked-scan machinery instead, each re-verified against the current tree rather than copied from SPEC's prose. SPEC.md's own front matter (lines 1-12, a reader's-guide header, not a record) was rewritten to fix a stale "P0–P31 implemented" status (the real figure is P0–P37; P38 was skipped by user direction) and its `docs/plans/` pointer, and to invert authority to ARCHITECTURE.md per D60/D61; no other line of SPEC.md's body, and no file under `docs/v1/plans/`, was touched. `tests/db/` → `tests/unit/` reconciliation (D66, P44 §8's own handoff item): metadata-cache.spec.ts and run-state.spec.ts move, not three files — preconnect.spec.ts measured at 23.0s of a 24.1s total and spawns real OS processes, so it stays; a collision found while planning this (Bun's shared module registry means two specs' inline `window` stubs race for which one "wins" for the whole test run) was fixed with one shared `tests/unit/support/window.ts` (commit 11) before the move (commit 12), proven both before and after: 52→63 pass, 0 fail. `bun run format`/`lint`/`typecheck` (five projects)/`bunx electron-vite build` stayed clean after every commit, and the Docker-free Playwright subset (`sqlite`/`startup`/`smoke`/`connections`/`workbench`) passed for real in this sandbox after every commit throughout. One deviation from the plan, forced rather than chosen: commit 2's four-line diff to `.github/workflows/{ci,release}.yml` (updating the same `docs/v1/PACKAGING.md` → `docs/PACKAGING.md` reference already fixed everywhere else) could not be pushed — this session's GitHub token lacks the `workflow` OAuth scope, the same limitation recorded against P25's CI step — so those two files were reverted out of that commit and the diff handed to the repo owner to apply by hand; every other file in that commit landed normally. |
| **P46 Disable unnecessary Chromium/Electron features** | Not a SPEC §10 deliverable — a user-directed, post-v1 phase: audit the app's Electron/Chromium surface and disable whatever this app doesn't use — renderer capabilities the app never needs, DevTools reachability outside development, spellcheck/autofill/form-data features, unused Chromium command-line switches, background-throttling and GPU features the app doesn't rely on, and any other default-on Electron/Chromium capability with no purpose here | The user's own framing: disable unnecessary Chromium and Electron features | Implemented per `docs/v1/plans/P46-disable-unnecessary-chromium-electron-features.md`, nine commits (`b6eac8d`…`7e55f8e`). Every disable is backed by a reproduction against the shipped renderer, not an inference from Electron's own hardening checklist: `window.open('https://example.com/')` returned a live handle with the window count going 1→2; `location.href` to a remote origin moved the real app off its own page and destroyed the workbench, with the `<meta>` CSP having no say over top-level navigation; every Chromium permission (notifications, geolocation, media, device APIs) was granted by default because no handler existed; `devTools: true` in a packaged build left `openDevTools()` reachable even though the keyboard shortcut path was already closed by `menu.ts`'s dev gating; and `session.defaultSession.isSpellCheckerEnabled()` was `true` on a stock window, spellchecking the connection dialog's password field the instant the eye toggle reveals it. All five now close, via one new `src/main/security.ts` (`rendererWebPreferences`/`hardenSession`/`hardenWindow`, its only `electron` import `import type` so it unit-tests with no mocking at all) plus `autocomplete="off"` on the one shared `TextField.vue` sixty call sites share, plus three Electron fuses in `electron-builder.yml` (`runAsNode`/`enableNodeOptionsEnvironmentVariable`/`enableNodeCliInspectArguments`, all `false`) closing the one remaining "debuggable outside development" path a Node inspector on main would otherwise leave open. Two findings kept this from being a blanket pass, both proven by running them: a deny-all permission handler broke `navigator.clipboard` — the grid's paste and `clipboard.ts`'s 38 `copyText` call sites — with no failure visible in this sandbox's Docker-free subset, so the fix is a two-entry allowlist (`clipboard-read`/`clipboard-sanitized-write`) shared by both the request and check handlers rather than a deny-all; and flipping `grantFileProtocolExtraPrivileges` off on a copied Electron binary bricked the app outright (the renderer's own `<script type="module" crossorigin>`, loaded over `file://`, is CORS-refused without it) — left on, with `scripts/verify-packaging.sh`'s new S6/S7 static checks (confirmed to actually fire, by breaking and reverting each) guarding both the three safe fuses and this one staying untouched. Three items were decided rather than disabled, each recorded as its own decision: background throttling stays on (nothing on the quit-flush or run-state-ticker paths depends on a hidden window's timers running unthrottled); hardware acceleration stays on (the grid's scroll budget depends on GPU compositing); `disableDialogs`/`safeDialogs` stay off the table (`window.confirm()` gates six destructive actions, and replacing them is a UI change for a future phase). No unused command-line switch existed to remove, and none was added — a `--disable-features` list is fail-open and untestable, and every capability it would target here either doesn't exist in this app or has a first-class, assertable Electron API instead. New `tests/ui/hardening.spec.ts` (Docker-free, ten scenarios) plus `tests/unit/security.spec.ts`/`menu.spec.ts` hold every claim for real in this sandbox except two, both honestly recorded rather than assumed passing: the three Electron fuses were only measured against a copied binary running the real `out/main/index.js`, not a packaged `.app`, and the WebGL scenario passes vacuously here because xvfb blocklists WebGL before the option is ever consulted — both need a `package:mac` run on a macOS box, which this sandbox cannot produce (no macOS, and the native Kafka rebuild `prepackage:mac` needs is blocked here regardless, per CLAUDE.md). `bun run format`/`lint`/`typecheck` (five projects)/`bunx electron-vite build` stayed clean after every commit, and the Docker-free Playwright subset plus `hardening.spec.ts` passed for real in this sandbox after every commit throughout. |
| **P47 `@tanstack/vue-virtual` spike** | Not a SPEC §10 deliverable — a post-v1 spike, gated by an evidence-based go/no-go decision (D14) rather than committed to unconditionally: replace `DataGrid.vue`'s hand-rolled row/column virtualization — manual viewport-size tracking, `visibleColumnRange`'s binary-search-plus-expansion-loop (P29) — with `@tanstack/vue-virtual`'s row/column virtualizers, bridging P29's existing 560px pixel-budget overscan behaviour into the library's `rangeExtractor` seam exactly rather than approximating it | An unmaintained, hand-rolled virtualization layer versus a widely-used library, weighed against the real risk that a library swap silently regresses the P29 invariants (overscan coverage, zero-mutation sub-row scrolls, DOM-cell bounds) it took a whole prior phase to establish | Implemented per `docs/v1/plans/P47-tanstack-virtual-spike.md`, three commits (`7b1e871`, `8dad3f2`, `097e4a6`). The row axis bridges exactly (`ceil(560 / 28) = 20` reproduces the old fixed overscan-row count character for character); the column axis bypasses the library's own `overscan` and supplies `visibleColumnRange`'s expansion loop unchanged as a custom `rangeExtractor` (`columnRangeExtractor`, `columns.ts`), so the same pixel budget and per-side cap apply. One regression surfaced and was fixed before commit: TanStack's default `observeElementRect` measures the scroll element's border-box size (`ResizeObserver`'s `borderBoxSize`/`getBoundingClientRect`), which — unlike the old code's `clientWidth`/`clientHeight` — doesn't subtract a visible scrollbar's own thickness; on `scroll_grid` (a horizontal scrollbar present) that ~12-15px discrepancy put the vertical overscan boundary on a knife's edge, mutating 65 DOM nodes on a 4px sub-row scroll and failing `budgets.spec.ts`'s zero-mutation assertion. Fixed with a custom `observeElementRect` (`DataGrid.vue`'s `observeScrollElementRect`) that measures `clientWidth`/`clientHeight` via its own `ResizeObserver` instead. D14's checkpoint: all five no-go criteria checked clean (sub-row zero-mutation, both overscan-coverage invariants, the DOM-cell bound, no D13 metric regression, `data-view.spec.ts`'s filtered-grid gutter numbers) and the go clause's bar is cleared twice over — `columns.ts` loses `visibleColumnRange`'s 43-line binary-search-plus-expansion-loop for the much smaller `columnRangeExtractor` seam, and all three D13 work-p50 metrics improved (big_rows vertical 2.4→2.2ms, scroll_grid horizontal 6.3→6.2ms, scroll_grid vertical/wide-table 6.2→5.1ms — see `docs/PERF.md` §2.1 for the full before/after). **Verdict: go.** `columnRangeExtractor` gained a dedicated unit spec (`tests/unit/column-range.spec.ts`, 7 cases: both caps engaging, both edges clamping, a single-column table, a table narrower than the viewport, a zero-column table, uniform and variable column widths) per D16; `budgets.spec.ts`/`perf.spec.ts` needed no edit at all (D17 — confirmed via an empty `git diff` against their pre-migration state). `bun run lint`/`typecheck`/`build` clean throughout; the full `bun run test:ui` suite passes except `memory.spec.ts`'s pre-existing RSS-budget finding (§2.2 of `docs/PERF.md`, D21 — confirmed via a clean-`HEAD` re-run to be unrelated to this phase). |
| **P48 Cross-view reusability audit** | Not a SPEC §10 deliverable — a user-directed, post-v1 audit-and-refactor of the six data-view kinds (grid/SQL, console, document/Mongo, keyvalue, stream, definition), their `ViewChrome`-based toolbars, and the backend engine-adapters layer, looking for logic and components that are duplicated rather than genuinely shared. Named examples to start from: the SQL grid's `DataGrid`/cell-editor pairing versus the console's read-only SQL result rendering (only partially unified so far, by P40); the Mongo document view's row/expand-collapse handling versus the console's Mongo result rendering (P46 added expand-all/collapse-all to the console separately rather than sharing the document view's own implementation); toolbar button ordering and composition, which currently differs across every `ViewChrome` consumer; non-UI logic (loaders, connection gates, mutation/error handling, projection/pagination helpers) revisited for drift accrued since P39's own pass; and the same duplication question applied to the backend adapters (postgres/mysql-family/sqlite/clickhouse/mongo/redis/kafka/sqs/s3/rabbitmq). The deliverable is an actual refactor extracting genuinely shared, configurable primitives where the duplication is real — not a forced merge where two things only look alike | User-reported: a fix or improvement made in one data view isn't reflected in its siblings, because what looks like one shared component is often two independently-maintained ones; queued as a follow-up to P39 now that six-plus phases of console/grid/Mongo work since P39 landed have had time to accrete new, un-audited duplication | Implemented per `docs/v1/plans/P48-cross-view-reusability-audit.md`, eighteen commits (`a642783`…`3ac99c6`). Zero-behavior-change discipline (D0) held for sixteen of the eighteen: every `data-testid` survives byte-for-byte except one deliberate pair-rename (`document-prev`/`document-next` → `document-pager-prev`/`document-pager-next`, step 15's shared `Pager.vue`), and only two commits change observable behavior, each its own commit and named up front — a superseded row-count response landing on a document/key-value/stream tab after a newer one already resolved (`countOpId`, ported from `grid/state.ts`'s existing guard) and the data grid's own 1px chrome border, dropped when it adopted `ViewChrome` like every other view. On the adapters side: a new `throwIfCancelled` in `errors.ts` — `assertNotCancelled`'s mid-flight sibling, the check re-run after an await rather than before a call starts — replaced twenty-six duplicated copies across eight adapters; a new `abort.ts`'s `withAbortRace` collapsed the six postgres/mysql-family callback-style abort/settle races; `shared/protocol/page.ts`'s `unpagedPosition` replaced ten copies of the same unpaged-position literal; `sql-text.ts` absorbed the SQL read path's keyset planning (`assertKeysetSupported`, `resolveFetchColumns`, `buildScanOrderBy`, then `buildKeysetPosition` collapsing three 28-line strategy/hasMore/nextToken/prevToken/position blocks across postgres/mysql/sqlite) and its count/filter logic (`whereClause`, `parseCountValue`), plus `primaryKeyFromIndexes`/`resolveKeyShape` moved out of postgres's and mysql-family's own catalog modules; `requireConnected` replaced ten copies of the same connected-handle guard. On the views side: `createRuntimeStore` grew `setActionError`/`toggleSearchOpen`/`setSearchOpen`, deleting five per-view copies; `viewOp.ts` gained `beginOp`/`applyLoadFailure`, adopted by grid/documents/keyvalue/stream/console's load paths (browse's own `loadSeq`-based supersession stayed separate, D18); `views/shared/page/store.ts` was rewritten into the one page-cache implementation behind all five page modules; `views/shared/immediateMutation.ts` now backs documents/keyvalue/stream's ten immediate-mutation writers; `views/shared/page/search.ts`'s `createMatchIndex` and `scan.ts`'s `emptyScan`/`tabularRowScanner`/`keyValueRowScanner` replaced each view's own copies of the find-toolbar's match index and row scanner. `DataView.vue` (the SQL grid) now mounts `<ViewChrome>` exactly like every other view — its hand-rolled `ViewHeader`/`.p-toolbar-rail` deleted, `DataToolbar`/`FilterToolbar` retargeted onto `ViewChrome`'s named slots — and a new `views/shared/page/Pager.vue` backs both the grid's and the document list's pagers. A new `views/shared/document/DocumentRow.vue` carries the Mongo document row's head (five `data-testid`s, nine CSS rules) shared verbatim between `DocumentView.vue` and `ConsoleResultGrid.vue`'s own read-only copy, each supplying only its own `#actions`/`#body` slot content. `views/shared/page/columns.ts` gained `columnHeaderTooltip`, `GUTTER_WIDTH` and `DEFAULT_COLUMN_WIDTH`, ending three different spellings of the same two constants across `DataGrid.vue` and `ConsoleResultGrid.vue` (a bare `56`, a literal `width: 56px` in CSS now a `--gutter-width` custom property, and each grid's own `?? 96` fallback). `biome.json`'s `views/<kind>/*` sibling-import group gained `browse`, the seventh view folder it had never covered since P41 added it — proven with a throwaway violating import, confirmed to fail and then pass once removed, the same way the rule already behaves for the other six kinds. `bun run lint`/`typecheck` (all four projects)/`bun run build` stayed clean after every commit; `bun test tests/unit` (81 tests) passed throughout; the `data-testid` diff was checked commit-by-commit against the plan's own predictions; the Docker-free Playwright subset (`sqlite`/`startup`/`smoke`/`connections`/`workbench`) passed for real in this sandbox both before and after the highest-risk steps (14's `ViewChrome` rewrite, 15's pager extraction). `data-view.spec.ts`/`mongo.spec.ts` and the full Postgres/MySQL/MongoDB/Redis/S3/Kafka/RabbitMQ/ClickHouse-backed suite are Docker-gated and were not run in this sandbox (Docker image pulls are blocked here, per CLAUDE.md); they typecheck/lint clean and need a real run in CI or on the macOS/Colima box before this phase is fully verified. |
| **P49 Vue/Node ecosystem library spike** | Not a SPEC §10 deliverable — a post-v1 spike, gated per-library by the same evidence-based go/no-go discipline P47 used for its own decision (D14), rather than adopting or discarding anything by default. Two parts: (1) revisit P47's own "go" verdict now that it's had time to prove out only on the SQL grid — decide whether the other views' shared `VirtualList.vue` primitive (document, keyvalue, stream, console results) should also move to `@tanstack/vue-virtual`, with outright removal of the existing adoption back on the table if a fresh look finds it doesn't hold up even there; (2) broaden into a general survey of well-known libraries in the Vue ecosystem (TanStack Query, TanStack Form, Pinia, VueUse, and others surfaced during the spike itself, not limited to those named up front) and their Node.js backend equivalents, each judged against this codebase's actual needs. The deliverable is a written per-library analysis — why it would help or why it wouldn't — not a mandate to add any of them | Direct follow-up to P47, whose go/no-go only ever covered the SQL grid in isolation; the same rigor needs to run both backward (does that existing adoption still hold up) and outward (would it, or anything else commonly used in Vue/Node projects, help the views and backend code that don't yet use it) before treating P47's decision as the final word | Implemented per `docs/v1/plans/P49-vue-node-ecosystem-library-spike.md`, plus a direct implementation follow-up on three of its own findings (greenlit by the user rather than spun into a separate phase, four commits `515fa2b`…`4ec3ed4`). The spike itself concluded no library is worth adopting: P47's `@tanstack/vue-virtual` adoption stays (one of its three recorded metrics actually moved outside D14's own noise band, not three, but the verdict holds on that one leg) and is **not** extended to `VirtualList.vue`'s other six call sites — every one of them is single-axis, the win TanStack demonstrated was specifically the column axis, and three of `VirtualList.vue`'s own features (a precomputed `rowHeights` prefix sum, in-flow sticky slots, a pixel-inset `scrollToIndex`) have no library equivalent. Pinia, TanStack Query, TanStack Form, VueUse, vue-router, a third-party component kit, `drizzle-kit` and `lru-cache` are each a no for a reason specific to this architecture (most sharply TanStack Query: the cache already lives in the engine process, every read is a logged operation, cancellation is server-side, and a cursor-paged page has no stable key). `@floating-ui/vue` is the one "revisit later." The survey's own real finds were two dependency-free defects, both fixed in the follow-up: `ConsoleResultGrid.vue` rendered every column of every visible row with no cap (F9) — it now shares DataGrid.vue's own `columnRangeExtractor`/`GUTTER_WIDTH`/`DEFAULT_COLUMN_WIDTH` behind a second TanStack column virtualizer, keeping its rows on `VirtualList.vue` (the conservative half of D4's own open question, since nothing on the row side showed a symptom); `KeyValueView.vue`/`StreamView.vue` rendered every loaded row directly with no virtualization at all, up to 10,000 rows (F7) — both now mount `VirtualList.vue`, closing a hole `keyvalue/search.ts`'s own doc comment had named. `OVERSCAN_PX`/`MAX_OVERSCAN_COLUMNS`/`observeScrollElementRect` moved out of `DataGrid.vue` alone into `views/shared/page/columns.ts` so both grids' column axes share them (P49 F3's own warning against a second copy). Along the way, P49's own D12 finding — five hand-rolled overlay positioners (`PopoverPanel.vue`, `AppTooltip.vue`, `ErrorPopover.vue`) already drifted from each other, one bug already shipped from it — was also greenlit rather than left on the "revisit later" list: `theme/anchoredPosition.ts` is one pure function parametrized by a `'menu'`/`'callout'` strategy replacing all three call sites byte-for-byte, proven by a dedicated unit spec asserting each site's own known before/after coordinates; `ContextMenu.vue` stays untouched (mouse-anchored, a deliberately different model). `bun run lint`/`typecheck` (all four projects)/`build` stayed clean after every commit; `bun test tests/unit` (90 tests) passed throughout; `bun test tests/db` reproduced the pre-existing 12-pass/10-fail baseline; the Docker-free Playwright subset passed after every commit, and a new `tests/ui/sqlite.spec.ts` scenario proves the console's column axis live rather than only typechecked — a 200-column single-row query renders well under 200 DOM cells and, after scrolling to the far right, still does while the last column's value is visible. `tests/ui/redis.spec.ts`'s own exact-count assertion on a 30-row list (larger than `VirtualList`'s default overscan) was rewritten to check the status line's loaded-count instead of a DOM node count, and a new DOM-node upper bound on a 5000-field Redis hash's own 100-row page proves that windowing is real too — both Docker-gated and unrunnable in this sandbox, confirmed compiling and skipping cleanly here; they need a real run in CI or on the macOS/Colima box, per CLAUDE.md. |
| **P50 UI test suite split: IPC-boundary backend tests and mocked-IPC frontend tests** | Not a SPEC §10 deliverable — a user-directed, post-v1 test-infrastructure phase: split `tests/ui/`'s per-adapter Playwright specs (`sqlite`/`redis`/`mongo`/`mariadb`/`mysql`/`clickhouse`/`rabbitmq`/`s3`/`sqs`/`kafka`) at the IPC boundary into two tiers instead of one. Only 2–3 specs stay as full end-to-end coverage (real Electron UI, real adapter, real data store), proving the whole stack still wires together. Every other adapter spec's scenarios split into (a) a **backend** test that drives the real IPC control-channel handler against a real (Testcontainers or file-backed) data store with no Electron renderer at all, and (b) a **frontend** test that drives the real rendered UI in Playwright/Electron with the IPC layer mocked, asserting rendering/interaction only. The one binding rule: the mock payload a frontend test injects and the fixture a backend test's real response is checked against live in the *same file*, in the same folder, so the backend test is what keeps the frontend's mocked contract honest — a frontend spec can never mock a shape the backend has stopped producing without that same file's own backend assertion failing first. The plan also has to resolve, with reasoning rather than by default: whether the new IPC-level backend tier joins `tests/db/`'s existing Testcontainers suite (`bun run test:db`) or is its own suite, and whether the resulting tests (backend and/or frontend) can run in parallel without flaking, given Playwright's and Testcontainers' existing serial/parallel configuration | User-directed: the current per-adapter `tests/ui/` specs conflate two different things behind one Electron launch — real backend correctness against a real data store, and real frontend rendering/interaction — so a frontend-only regression can only be caught by paying for a full adapter+Electron run, and a backend contract drift can hide behind a frontend mock that nothing keeps in sync. Splitting at the IPC boundary, with the mock/fixture living in one shared file per scenario, makes each tier fast and honest on its own terms while a small top-level slice keeps proving the full app still works | Implemented per `docs/v1/plans/P50-ipc-boundary-test-split.md`, ten commits (`e931759`…`72e5162`) — the first six landed the initial adapter splits and the kept-anchor labeling; the next two (`0705b69`, `eda5f50`) landed the ClickHouse split and the Opus-driven db-vs-ipc duplication review described below; the last two (`39dee27`, `72e5162`) landed the Kafka split described further below (a `git add` pathspec error split what was meant to be one commit into two — the first landed only the `tests/ui/` deletions, the second added the new `tests/ipc/kafka/` files themselves). The backend tier is its own suite (`tests/ipc/**/*.backend.spec.ts`, `bun run test:ipc:be`) rather than joining `tests/db/` — a repo convention now, not just this phase's choice: `bun run test:db` is a directory glob Bun itself would try to run these files under and fail on the spot, and `tests/db/`'s own scope (the adapter against a real container) is a different subject from this tier's (the four layers above the adapter — control channel, data-op dispatch, tree/metadata cache, port framing). Each adapter folder holds three files sharing one name: `<adapter>.backend.spec.ts`, `<adapter>.frontend.spec.ts`, `<adapter>.fixture.ts` — colocation of both halves and the fixture is the binding rule, not a directory split (also now a repo convention, D1/D2). Mocking mechanism: `window.kira` is deeply frozen and non-configurable (confirmed empirically) — renderer-side IPC mocking is impossible — so the control channel is mocked by swapping `ipcMain` handlers in the main process via `ElectronApplication.evaluate`, and the bulk-data port is mocked by posting a fresh `MessageChannel`'s `port1` through the same `{__kira:'port'}` door the production code already listens on for reload/reconnect. Landed for real, against real containers pulled via `mirror.gcr.io` (a Docker Hub read-through mirror this sandbox's network policy does not block, unlike Docker Hub directly or `quay.io`) — a discovery of this phase, now recorded in `CLAUDE.md`: **mariadb** (pilot, commit 1, plus `tests/ipc/support/harness.spec.ts`'s Docker-free proof of the harness itself against a temp-file SQLite fixture), **redis** (commit 2, first non-tabular `KeyValuePage` shape, first real use of the ordered-request-log), **mysql** (commit 3, backtick-quoted filter-by-value, the "Routines" folder as a frontend-only grouping over real leaf-kind nodes — there is no backend "folder" node kind at all), **rabbitmq** and **sqs** (commit 4, the two message-broker shapes; sqs brings `StreamPage`'s `pagination:'batch'` into the fixture vocabulary; this commit also generalized the ordered-sequence match cursor from the port mock to the control mock, and excluded a request's `tabId` from the control mock's arg-matching key — a per-tab UUID the renderer generates at open time, never reproducible from a fixture, the same reasoning the port mock already applied to `opId`/`tabId`). Commit 6 added a header comment to each of the three kept full-stack anchors (`sqlite.spec.ts`, `mongo.spec.ts`, `s3.spec.ts`) naming what it is now the sole proof of, with zero assertion changes.

**ClickHouse landed too, in a follow-up commit, rather than staying deferred**: `@testcontainers/clickhouse` hardcodes a `ulimit` override this sandbox's own fixed `ulimit -Hn` ceiling cannot satisfy even as root, so a container started the stock way never came up — but `hostConfig` is `protected` on testcontainers' own `GenericContainer`, so a small subclass (`tests/ipc/clickhouse/container.ts`'s `NoUlimitClickHouseContainer`) that clears `hostConfig.Ulimits` in its constructor unblocked a real container here, and the backend spec captured a real fixture against it (one adapter-specific non-determinism found and frozen per D6: a materialized view's auto-generated `.inner_id.<uuid>` backing-table name, a fresh random UUID per container). `tests/ui/clickhouse.spec.ts` was deleted once both halves were verified.

A follow-up Opus-driven review compared every `tests/db/<adapter>.spec.ts` against its new `tests/ipc/<adapter>/<adapter>.backend.spec.ts` twin (mariadb, mysql, redis, rabbitmq, sqs) for real duplication rather than assuming either the concern or its absence. Verdict: mostly theoretical — the two tiers test different things by construction (`tests/db/` is exhaustive adapter-edge coverage, `tests/ipc/`'s backend half is one linear fixture-capturing scenario per adapter), and most surface-similar pairs (a `serverVersion` regex, `caps` equality, cancellation) are claims about different layers wearing the same expected value, not duplicates — `redis`'s `truncated` field is the sharpest example, asserted as `undefined` at the adapter level and `false` at the IPC level, both correct, because `tree-service.ts` normalizes what the adapter omits. Three assertions across two adapters were confirmed as genuine zero-value duplication and removed: rabbitmq's nameless-default-exchange check and its poll routing-key value, and sqs's empty-queue count value/exact check — none of it in `tests/db/` itself (P50 D1 stays honored), and no fixture changed.

**Kafka landed too, in its own follow-up commit** — its blocker was never the image pull (`confluentinc/cp-kafka` pulls fine via `mirror.gcr.io` like every other broker), it was the native ABI rebuild (`CLAUDE.md`'s Native Kafka driver section, P32). Two things changed since that section was first written: the proxy 403 fetching Electron's own C++ headers, present earlier this phase, was gone on retry (proxy state in this environment isn't permanent); and past that, `@confluentinc/kafka-javascript`'s own build always tries to vendor-build zlib/libcurl/libcrypto/zstd from source (`util/configure.js`'s hardcoded `--install-deps --source-deps-only`) regardless of what's already on the system, and two of those source hosts (`zlib.net`, `curl.se`) are ones this sandbox's proxy hard-blocks — confirmed via the proxy's own status log, not assumed. `util/configure.js` reads a `CKJS_LINKING` environment variable; set to `dynamic`, it skips the vendored build entirely and links against the system's own dev packages instead, avoiding both blocked hosts. With that rebuild done, `bun run test:db:kafka` (all 21 cases) passed against a real broker, unlocking the same real-fixture split every other adapter got: `tests/ipc/kafka/kafka.backend.spec.ts` drives the real tree (topics ungrouped, a consumer group's own root-level node, "Consumer groups" a frontend-only folder over it — D15's own `mysql`/`clickhouse` precedent), the partition-filter popover's live `children()` call, an `offsetWindow` stream read, and both topic/consumer-group definitions. Two adapter-specific non-determinism findings, both frozen per D6: a read fans across both partitions and interleaves them by arrival rather than key/offset order (confirmed via two independent write-mode runs disagreeing on order; sorted by key before the fixture is written, same reasoning as redis's own HSCAN-reordering finding), and the consumer group's own coordinator host:port (a fresh Docker-assigned container hostname and a fresh host-mapped port every run). `tests/ui/kafka.spec.ts` and its own `tests/ui/support/kafka.ts` were deleted once both halves were verified.

`bun run lint`/`typecheck` (five projects, `tsconfig.node.json` gained one `include` entry for `tests/ipc/**/*.ts`, no new root `tsconfig.json` reference) and `bunx electron-vite build` stayed green after every commit; the `e2e` Playwright project (`tests/ui/`, `workers:1, fullyParallel:false`) was re-run for sanity after each deletion and stayed green; the `ipc-frontend` project (`tests/ipc/**/*.frontend.spec.ts`, `fullyParallel:true`) passed all eight specs together in this sandbox with no Docker and no native driver — kafka's frontend half mocks the IPC layer entirely, so it needs neither the broker nor the native driver at run time, only its backend half does.

`memory.spec.ts` was then removed outright (`d23121e`), not fixed: §2.2's total-RSS budget stayed red on every D21 lever already exhausted, over budget purely on Chromium/Electron process overhead no app-level change can address — `measure.ts`'s now-unused `sampleRss`/`sampleRssSeries`/`formatRssSeries`/`ProcessSample`/`RssSample` went with it, and `docs/PERF.md` §2.2 records the structural finding in the deleted test's place.

With both IPC-boundary halves confirmed real Electron GUI runs (backend: `ELECTRON_RUN_AS_NODE=1` for ABI-correct native-module loading only, no window; frontend: a full `_electron.launch()` window, same as `tests/ui/`'s own specs), the old `tests/ui/` name no longer described what ran there uniquely — every other Playwright suite under `tests/` now also drives a real Electron window. The whole directory was renamed to `tests/e2e/` to match the Playwright project already named `e2e` (`testDir: './tests/ui'` → `'./tests/e2e'`), with every functional import in `tests/ipc/*/*.frontend.spec.ts`, `playwright.config.ts`, `tsconfig.node.json`, `package.json`'s `test:ui`/`pretest:ui` scripts, `.github/workflows/ci.yml`'s `ui-smoke` job, and every living doc/comment reference updated to match; `docs/v1/plans/*`'s own historical prose and this row's own prior paragraphs above (accurate to what the directory was actually called at the time each described action happened) were deliberately left saying `tests/ui/`. |
| **P51 Wails (Go) shell + Go app core + Node engine child process, migration spike** | Replace P51's former Tauri slot with a materially different architecture, decided by the user: investigate migrating the app off Electron onto **Wails (Go)**, with a **partial** backend migration rather than a total one. Wails/Go becomes the native shell *and* absorbs **all** of `src/main` into the same process — window/menu/lifecycle/security as native Go idioms, plus the 14 IPC handler modules, the SQLite storage layer (10 repos, 9 schema modules, 5 `.sql` migrations), settings/layout/tabs/saved-queries/filters/connections persistence, the op log, pre-connect scripts, the tree service, connection state, credential encryption and logging. **`src/engine` (119 files / 14 743 lines, eleven DB adapters, cache, scheduler) stays exactly as it is, in TypeScript**, running as a genuine plain **Node.js** child process — a real Node runtime binary vendored into the bundle with the engine's `.js` and `node_modules` shipped as normal loose files beside it, spawned from Go's `os/exec` the way `src/main/engine-host.ts` forks it from Electron today. Explicitly ruled out by the user: Rust/Tauri ("some more code might migrate in the main app and i prefer go"), and any non-Node JS runtime or single-file compilation (Bun, Deno, Electrobun's Cottontail, `pkg`, Node SEA, `bun build --compile`). Install size is explicitly not a concern, which is what licenses vendoring a real Node runtime and removes the self-extract-then-`dlopen` packaging/hardened-runtime risk the Tauri plan had to grapple with; backward compatibility with existing `kira.sqlite` rows is explicitly not required, so the Go-side storage/settings schema is defined fresh with no zod-`.default()`-style compatibility machinery to replicate. The motivation is unchanged from P20's and the Tauri slot's: `docs/PERF.md` §2.2's ~620MB of Chromium/Electron baseline overhead against a 350MB budget, which no app-level lever can reach (`tests/e2e/memory.spec.ts` was removed rather than kept red, commit `d23121e`). The spike's deliverable is a written report: the renderer↔Go bridge design (how 61 wire-protocol channels map onto Wails bindings, and whether any primitive avoids a full JSON round-trip for bulk data), the Go↔Node engine transport design (stdio pipes vs. a Unix domain socket, framing the existing transport-agnostic `PortRequest`/`PortResponse`/`PortEvent` protocol — new code on both sides, and the one change `src/engine` needs), the bundling/two-binary signing story given Wails has no sidecar mechanism, engine lifecycle and supervision, the Go-side Keychain seam replacing `safeStorage`, the E2E-testing story, a real memory measurement, and a completed per-module disposition — plus a go/no-go recommendation. Incrementally migrating individual DB adapters out of the Node engine into native Go later (the per-kind `adapters/registry.ts` loader map is already the right shape for it) is named as the natural future direction this architecture leaves open, and is explicitly **not** designed in this phase | **Superseded for implementation by P52.** The spike itself never went further than the plan
document below — **no Go or Wails toolchain install, no scaffold, no code change, no dependency
addition and no measurement were performed under this row's own phase number**. The user signed
off on the recommendation, and implementation proceeded as **P52 Wails/Go migration** (own row
below) rather than under a "P51 implementation" label — see that row for what was actually built.
See `docs/v1/plans/P51-wails-go-node-engine-spike.md`. That document is this phase's entire output
so far: it **required the user's explicit sign-off before any implementation begins** — the word "spike" in the phase name does not exempt it from `CLAUDE.md`'s Opus-plans-then-sign-off-then-implement loop. Almost every claim about Wails itself is flagged as best-current-knowledge for a spike stage to confirm directly: `wails.io`/`v3.wails.io` are blocked by this environment's egress proxy, so the official documentation was never read. Three things were established from GitHub instead and are labeled as such in the plan: Wails has **no first-class sidecar/external-binary mechanism** (discussion #3021 asks for a Tauri-sidecar equivalent and is answered only with `go:embed`, which a commenter correctly notes is not the same thing), so spawning the Node child and bundling the vendored runtime are both hand-rolled; Wails v3's **default binding transport is a `fetch()` to a local `/wails/runtime` endpoint with a `JSON.stringify` body** (read from the v3 runtime source), i.e. a full JSON round-trip by default, though the same source exposes a `setTransport()` override worth investigating for bulk data; and **no first-party WebDriver story surfaced** (discussion #4205 recommends pointing Playwright at `wails dev`'s `localhost:34115`, which is not shell-integration coverage). The E2E decision carries over from the deleted Tauri plan unchanged and unconditionally — the UI suite moves fully to Playwright's isolated `webkit` tier, permanently giving up native menu/window/lifecycle coverage, real-IPC-bridge correctness, real Keychain coverage and `src/main/security.ts`'s navigation-lock posture. Two things this architecture improves over the deleted Tauri-sidecar plan, stated in the plan itself: crash blast radius returns to today's already-accepted shape (settings/tabs/layout/op log live in the Go process, so losing the engine means "reconnect your databases" — `engine-host.ts`'s existing no-auto-respawn policy — not "the app cannot save anything"), and the credential seam is resolved rather than relocated (Keychain access becomes ordinary in-process Go code, no privileged cross-process surface). Wails being a smaller, less mature project than Tauri is named as a real, accepted trade-off given the user's explicit Go-over-Rust preference, not something the spike relitigates. This row supersedes the former "P51 Tauri + Node sidecar migration spike"; P20's own row and its out-of-scope verdict are unchanged |
| **P52-P57 Wails/Go migration, implemented** | P51's own recommendation, carried out: replace the Electron shell with a native Wails v3 (Go) one, absorbing all of `src/main` into Go (window/menu/lifecycle/security, the IPC handler surface, SQLite storage, settings/layout/tabs/saved-queries/filters/connections persistence, the op log, pre-connect scripts, the tree service, connection state, credential encryption, logging), while `src/engine` (the eleven DB/queue/object-store adapters, in TypeScript) stays exactly as designed, running as a genuine Node.js child process under a real vendored Node runtime shipped beside the app — not Electron's, not the system one. Six phases, each with its own Opus plan under `docs/v1/plans/`, in order: P52 (Go app core scaffold), P53 (Go storage core), P54 (Go enginehost / stdio transport), P55 (Go application services / Wails bindings), P56 (native shell parity), P57 (the cutover itself, milestones M1-M8) | Not yet planned as a single SPEC row when P52 started — this row exists because `docs/v1/SPEC.md` §14 owed one for P52 and never got it, and P57's own M8 documentation milestone is what finally wrote it, covering all six implementation phases together rather than leaving five of them with no SPEC.md record at all | Implemented per `docs/v1/plans/P52-wails-go-migration.md`, `P53-go-storage-core.md`, `P54-go-enginehost-stdio-transport.md`, `P55-go-application-services.md`, `P56-native-shell-parity.md`, `P57-cutover.md` (plus `P57-e2e-revisit.md`, a mid-flight amendment) — see those documents and `CLAUDE.md`'s per-phase "implementation findings" entries for the full detail; this row is a summary, not a replacement for either. **P52** built the real Go application (`shell/`, Wails `Options`, asset embedding) and ported the whole of `src/main`'s domain logic to Go, including a new credential cipher (AES-256-GCM under the app's own key, `kira:v2:` envelope, replacing Chromium `safeStorage`'s AES-128-CBC — deliberately not migrated from `kira:v1:`, a fresh-database cutover) and a deliberately Electron-build-distinguishable bundle identifier (`com.kirathecat.kira-studio-shell`) for the P52-P56 coexistence window. **P53** built the Go storage repos layer (matching the prior Drizzle-backed TypeScript repos one-for-one) and its migrations, confirming `./internal/...` needs nothing but the Go toolchain. **P54** built the Go-side engine-host speaking the engine's stdio-framed wire protocol (replacing Electron's `parentPort`), fixing three real concurrency bugs in the process (a `writeFrame`/stdin mutex deadlock, a `cmd.Wait()` race with the stdout reader, and a shared-channel bug that split push events between subscribers instead of delivering each to all of them), plus a stdout-is-the-frame-channel-not-a-log-sink fix inside `src/engine` itself, and confirmed the bundled engine runs correctly under a bare Node with no load-bearing Electron coupling. **P55** built out the Wails-bound services surface (Settings/Layout/Tabs/Connections/Tree/Engine/Ops/Filters/Files/Queries/Lifecycle) and established the pinned-`wails3`-version discipline this whole migration depends on; `resolveEngine()` still pointed at a ping fixture rather than the real engine at the end of this phase, deliberately out of scope. **P56** landed native-shell parity work (the deferred-emitter/deferred-dialogs pattern for Wails' construction-order constraints, macOS reopen-event wiring, accelerator/modifier-key fixes, a bridge-level argument-validation convention) and, critically, switched `resolveEngine()` over to the real bundled engine for the first time — which is what first exposed the finding P57 was scoped to close: the renderer's own engine-status logic still expected Electron's old `MessagePort` handoff, so the status pill stuck on "connecting" forever with nothing actually broken underneath. **P57** (M1-M8) is the cutover itself and by far the largest single phase: M1/M2 rewrote `src/renderer/bridge/{control,port}.ts` onto Wails' generated bindings (control plane) and one named `JSONStream` (data plane), replacing `contextBridge`/`MessagePort` entirely, and fixed a `JSONStream` JSON-round-trip bug that silently dropped `TextColumnChunk`'s `TypedArray`s (`reviveChunks`); M3 wired the real bulk-data pass-through (`bridge/stream.go` forwards bytes verbatim, no parse or copy — the architecture's own documented invariant); M4 was the C1 boot-check milestone, proving both the Electron and Wails builds still worked side by side, the property that made M5 safe; M5 rebuilt the entire test tier (`tests/ui/` — 36 specs against real WebKit, both wire planes mocked; `tests/e2e-real/` — 2 specs against a real `-tags server` Go binary; `tests/e2e/` retired outright, its 23 specs ported or written off with a named disposition per spec — sqlite/postgres wiring recovered by `tests/e2e-real/`, S3's file-write contract recovered by a `tests/db/` case, MongoDB's own anchor value accepted as a real, permanent loss, `hardening.spec.ts`/`startup.spec.ts` deleted outright for having no Wails analogue at all; `tests/electron-db/` folded into `tests/db/kafka.spec.ts` under the vendored Node), surfacing three genuine bugs along the way (a mock fixture hardcoding `byteSize: 0` that made every retained-bytes leak assertion pass vacuously; a real `DataGrid.vue` performance bug where an unmemoized `v-tooltip` binding rewrote every header cell's DOM attributes on every scroll tick; a ported test whose own scroll-position constants silently stopped testing anything once its mock fixture's table was smaller than the real one) plus two Go-test-cleanup passes cutting the suite from 60 to 23 files against a much stricter "advanced/complex/deeply-nested logic only" bar, now written into this file's own testing conventions; M6 deleted `src/main/`, `src/preload/` and `src/engine/index.ts` (plus two Node-side unit tests whose subject had already moved to Go and gone untested-as-dead); M7 removed Electron from the build entirely (`package.json`'s scripts collapsed, six now-dead dependencies removed including a `drizzle-orm` nobody had noticed go dead in M6, `electron.vite.config.ts`/`electron-builder.yml`/`scripts/native-electron-build.sh` deleted, the shipping bundle identity landed everywhere it's mechanically load-bearing including two places no prior plan document had enumerated — `CFBundleExecutable` and `AppProcessSet`'s own process-matching needle — `scripts/sign-bundle.sh`/`scripts/verify-packaging.sh` rewritten against the real Wails bundle layout with an honestly-flagged, still-open Kafka-native-module packaging gap, and CI workflow updates staged outside `.github/workflows/` pending a push from a session with the `workflow` OAuth scope this one lacked); M8 is this documentation pass. Two findings from this work are worth naming here rather than only in `CLAUDE.md`: a real, measured memory/CPU regression versus the old Electron architecture (the Go↔Node engine's stdio protocol forces `JSON.stringify` of bulk typed-array data that Electron's real `MessagePortMain` structured clone carried natively — roughly 11x text inflation, 48x transient heap inflation for a 100KB chunk, empirically measured, not fixed in this phase and the most likely reason P58 exists), and a real Go performance bug fixed along the way despite being outside this migration's own core scope (`AppProcessSet` was scanning the whole system process table once per search needle instead of once per measurement tick, found while investigating an unrelated user report of high CPU usage). What is explicitly still open going into P58/P59: the Kafka native-module packaging gap named above; `src/shared/protocol/ipc.ts`'s `KiraApi`/`AppInfo` interfaces, dead leftovers from the pre-Wails `contextBridge` protocol that a full audit never got scheduled for (the one remaining non-comment "electron" reference in `src/`); and the CI workflow files themselves, still pending a `workflow`-scoped push as of this row being written. |
| **P58 DB adapters in Go, Node engine sidecar removed** | Migrate the per-kind DB adapters (postgres, mariadb, mysql, clickhouse, mongodb, redis, kafka, sqs, s3, rabbitmq — `src/engine`'s TypeScript, running as a Node child process per P51's own deliberately-partial scope) into native Go, and remove the Node engine subprocess entirely: no vendored Node runtime, no stdio/JSON transport to a sidecar, adapters called in-process from Go | Not yet planned — queued. The natural continuation of P51-57's Wails/Go migration, which kept `src/engine` as a Node sidecar rather than a total backend rewrite specifically to bound that phase's scope (P51's own row: "Incrementally migrating individual DB adapters out of the Node engine into native Go later … is named as the natural future direction this architecture leaves open, and is explicitly not designed in this phase"); this phase is that direction, removing the one cross-process boundary P57 leaves standing and the packaging/lifecycle complexity it carries (a second vendored runtime, stdio framing, engine supervision, per-driver Go library availability to confirm one adapter at a time). **Implemented, in six sub-phases each with its own Opus plan under `docs/v1/plans/`** — P58a (Go substrate + Postgres, the pathfinder and checkpoint C1), P58b (MySQL/MariaDB/SQLite/ClickHouse), P58c (MongoDB/Redis), P58d (SQS/S3), P58e (Kafka on `github.com/twmb/franz-go`, checkpoint C2), P58f (the cutover: deletes `src/engine`, `enginehost` and the vendored Node runtime entirely) — see `docs/v1/plans/P58-go-native-adapters.md` (the parent plan, since amended in place per each sub-phase's own findings rather than rewritten) and `CLAUDE.md`'s per-sub-phase "implementation findings" entries for the full detail. **This Deliverable column names two things the implementation did not do, left as originally written per this folder's own never-retro-edit rule (`docs/v1/README.md`)**: `sqlite` is missing from the adapter list (it migrated to native Go alongside the other nine, in P58b) and `rabbitmq` is listed as an adapter to migrate when it had already been dropped as a supported connection kind before this phase started, so it was never touched. The ten kinds actually migrated to native Go: postgres, mariadb, mysql, sqlite, clickhouse, mongodb, redis, kafka, sqs, s3 — `nativeKinds` reached ten of ten at P58e's own M9.3 commit. Driver picks, one per engine: `pgx/v5`, `go-sql-driver/mysql` (mariadb/mysql), `modernc.org/sqlite` (sqlite — pure Go, no cgo, also adopted for the app's own storage layer, closing the last cgo dependency in the product's own code), a hand-rolled `net/http` client (clickhouse), `mongo-driver/v2`, `go-redis/v9`, `franz-go`+`kadm` (kafka), `aws-sdk-go-v2` (sqs/s3). Two checkpoints proved the migration's safety properties for real rather than by inspection: **C1** (coexistence, after P58a M5) proved one native Postgres connection served end to end while the Node child still served the other nine kinds in the same session; **C2** (zero-traffic, before P58f M10) proved all ten kinds passed a full manual pass with the Node child spawned, idle and answering nothing, confirmed by a request-scoped warning counter reading zero. The Kafka native-module packaging gap P57 left open closed for real in P58e, then became moot entirely once P58f deleted the Node engine it would have needed. Net capability change: one narrow security-posture loss (MySQL's `allowPublicKeyRetrieval`, which `go-sql-driver/mysql` has no equivalent for) against several gains (Kafka regains `DescribeConfigs`/`DescribeCluster`, lost since P32; SQLite gains real server-side cancellation; MongoDB's document view renders whole-number doubles with their real on-disk type tag instead of losing it in a lossy JS round-trip; a packaged build can connect to Kafka for the first time ever). P58f M10 deleted `src/engine` (110 files, 13 637 lines), `enginehost`, `enginetest`, the vendored Node runtime, `scripts/vendor-node.sh`, and every packaging step that referenced any of them; `tests/db/` survives as a shared fixture corpus rather than a spec suite, and `tests/ipc/`'s backend half moved to a Go fixture generator (`shell/internal/ipcfixture`) that still writes the same `*.fixture.ts` modules the frontend specs read, keeping the tier's anti-drift guarantee intact. Bundle size, cold start and total RSS with no vendored Node were **not available in this session** — no macOS hardware — and are recorded as such, not guessed at, in `docs/PERF.md` §3. |
---
