...
| **P51 Electrobun migration spike, reopened** | Reopen P20's out-of-scope verdict under new, user-pre-accepted premises: lose `tests/e2e/`'s full shell-integration fidelity outright, but keep a BE and an FE tier surviving in some form; FE moves to Playwright's own `webkit` project running the built renderer bundle in isolation (a fake `window.kira`, no real Electrobun window at all) instead of trying to solve the WebDriver/CDP gap; the bulk-data architecture regression (P20's dealbreaker #2) is accepted only if the spike's own alternatives analysis (shared memory, a Bun-native main↔engine channel bypassing the webview bridge entirely, chunked/streamed responses) turns up nothing better; every other disadvantage P20 found (realities #1–15) gets re-audited against these premises and reported, not just the two original dealbreakers | Not yet planned for implementation — plan only, see `docs/v1/plans/P51-electrobun-migration.md`. That document is this phase's entire output so far: no Electrobun install, no code change, no `tests/e2e/` spec triage has been performed. It also surfaces one disadvantage P20 didn't have to weigh under these premises — `budgets.spec.ts`/`perf.spec.ts`/`startup.spec.ts` call `app.getAppMetrics()`/`app.evaluate(() => process.uptime())`, Electron-only APIs with no Electrobun counterpart, so that coverage doesn't move to either tier, it's just gone unless Electrobun exposes an equivalent — and that `hardening.spec.ts`/`secrets.spec.ts` exercise `safeStorage` (P25's credential encryption), which would need a from-scratch replacement under Electrobun. Requires the user's explicit sign-off on the finished spike report before any implementation begins |
