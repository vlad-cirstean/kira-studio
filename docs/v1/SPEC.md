...
| Kafka | cluster → topics (ungrouped), consumer groups (folder) | stream | offset window per partition | end-offset − begin-offset | stop consumer, `AbortSignal` |
...
| **P29 Grid/cell-view scroll rendering gap** | Scrolling either the data grid or the cell editor's own content quickly leaves a visible blank gap before the newly-scrolled-to content renders — worse horizontally than vertically. Root-cause the virtualization/paint timing (prefetch window, row-height math, horizontal column virtualization if any) and close the gap rather than papering over it with a spinner | A perceptible responsiveness regression in the app's two highest-traffic surfaces; standalone from every other phase in this batch |
