...
the fact survives the trim rather than being deleted — `AGENTS.md`'s own rule that an app fact
...
- **`views/shared/slick/selectionEdges.ts`** carries whatever unit coverage
  ...
  changed. If it has none, none is added — `AGENTS.md`'s bar; the function is geometry the grid's
...
- **No new unit test for D1, D3, D7, D9, D11.** Each is a CSS value or a string; there is nothing
  with interacting rules to guard, and `AGENTS.md` names exactly this class as "gets nothing".
...
- **How the mode tabs look on a real Mac.** D5's test is the substitute, and it is written to fail
  ...
  not installed here and `frontend/bindings/**` is generated by it (`AGENTS.md`).
