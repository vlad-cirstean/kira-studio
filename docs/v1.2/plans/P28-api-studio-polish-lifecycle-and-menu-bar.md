...
### F9 — **Corrected during implementation: there is no Mongo copy affordance that produces plain JSON, and no Mongo copy *button* anywhere**

This finding originally claimed `views/console/resultMenu.ts`'s `rowAsJsonMenu` (`Copy as JSON` /
`Copy all as JSON`, both `prettyJson`) was reachable for a Mongo result. Checked properly against
`ConsoleResultGrid.vue`'s own three branches, it is not: `onDocumentRowContextMenu` (the Mongo
branch) calls `mongoDocumentRowMenu`, and `rowAsJsonMenu` has exactly one call site,
`onKeyValueRowContextMenu` — the Redis branch its own comment says it is for.

What Mongo actually offers, enumerated exhaustively:

| Surface | Items |
|---|---|
| `views/documents/menu.ts` `rowMenu` (data view) | `Copy document ▸` {Shell mode, Canonical Extended JSON, Relaxed Extended JSON}, `Copy _id` |
| `views/console/resultMenu.ts` `mongoDocumentRowMenu` (console) | the same submenu, plus `Copy all ▸` with the same three |

Every one is explicitly labelled with its format; two of the three preserve types (shell
`ObjectId(…)`, canonical `{"$oid": …}`) and the third says "Relaxed" on its face. There is no
unlabelled "copy", and no copy **button** at all — not in `DocumentView`'s toolbar (checked: json,
symbol-number, list-selection, expand-all, collapse-all, add, search, history, edit, trash), not in
`DocumentRow`/`DocumentTree`, and not in the cell-editor dock (which has no clipboard call of any
kind). `copyText(` has 30 call sites across the app and none of them is a Mongo button.

**Consequence: item 9 is not implemented.** The reported control does not exist in this tree, so
there is nothing to change without guessing which of the three labelled formats the user meant and
rewriting it — which would be inventing a defect rather than fixing one. Reported back as
investigated-and-not-found, with the table above, so the next report can name the surface. See D9.
...
### D9 — **Not implemented, deliberately: no change without a surface**

F9's correction removes this decision's premise. Every Mongo copy affordance in the tree is a
labelled format, two of the three preserve types, and there is no copy button. Changing one of the
...
**(b) Teleport — reconsidered during implementation and NOT done.** The plan proposed teleporting
`PopoverPanel` and the autocomplete popup out of any ancestor stacking/clip context. Three things
argue against it, and together they outweigh the theoretical clip F17(b) describes:

1. **Neither is a reported surface.** The report names "right-click context menus and tooltips".
   `ContextMenu` and `AppTooltip` are both already teleported (`ContextMenu.vue:236`,
   `AppTooltip.vue:50`), so the two genuinely-reported surfaces are already immune to an ancestor
   clip. What was actually wrong for them is (a) — no `size()`, hence no scroll — and the z-index
   collision (c).
2. **Both are `position: fixed`,** so an ancestor has to establish a containing block
   (`transform`/`filter`/`contain`/`will-change`) before a clip is even possible. No such ancestor
   was found on any current consumer.
3. **The cost is real and immediate.** `PopoverPanel` resolves its anchor as
   `backdropEl.parentElement`, which teleporting invalidates — so every one of its nine consumers
   would need an explicit anchor — and every `tests/ui` selector that scopes a popover by its host
   (`[data-testid="cell-editor"] …`) would break at once.

Recorded here rather than silently dropped: if a popover is ever reported as clipped, the fix is a
non-teleported zero-size anchor probe left in place plus a teleported backdrop, which keeps the
"no consumer passes an anchor" property. Not needed today.
...
2. **D15c's alias→faker rewrite** — **in Go, not `packages/api-core`**, corrected during
   implementation: the Postman importer is `apps/kira-studio/internal/postman`, so that is where the
   rewrite has to live and where its test belongs (`internal/postman/aliases_test.go`). It rewrites
   user data at import across five field families, must leave an unmapped `$name` untouched, must
   not rewrite a longer name sharing a prefix or a reference with inner spaces, and must not consume
   the tail on an unterminated `{{` — exactly the "mapping table applied by a walker" shape the bar
   names. A **second** test comes with it, in `packages/api-core/test/go-ts-api-parity.spec.ts`:
   the 58-entry table now exists in two languages, and drift is silent in the worst direction (an
   alias missing from the Go table imports verbatim and then looks, to a user this app no longer
   teaches the `# P28 — Seventh user-driven batch: Api/Studio polish, floating-surface hardening, the menu bar, and the window's webview process
...
1. **D12's curl detection predicate** (`packages/api-core`, alongside `http-curl.spec.ts`). It is a
   ...
   sits on a *destructive* path: a false positive silently replaces the user's whole request instead
   of pasting text. The parser it guards is already tested; the guard is not.
 spelling to, like a reference nothing knows about). Same technique the three
   existing parity describes already use — read the Go source as text, compare against the TS table.
