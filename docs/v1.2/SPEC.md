...
| **P4 Collections: Postman-format storage, left-panel tree, import/export** | The real Postman Collection format (the actual JSON schema Postman itself writes, v2.1) **is this app's own on-disk storage format for collections** — not a bespoke internal format that Postman files are merely converted to/from at the import/export boundary. Every collection this app creates or edits is a real Postman collection file on disk, so importing is just reading one in and exporting is just handing one back out (or, in the simplest case, pointing at the file that's already there). Surface collections in the left panel (built on P1's modularized panel) as a folder/request tree | The organizing structure the rest of the feature hangs off — history, curl, and the timeline all operate on a saved request, and a request needs somewhere to live first. Storing the real format natively (rather than a bespoke format with a conversion layer) is what makes a Postman collection edited in this app remain a valid Postman collection everywhere else, with import/export as a near-trivial consequence rather than a lossy translation |
