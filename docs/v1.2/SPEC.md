...
| **P12 Studio/Http modularization & separability audit** | Go back over every phase's output (P1-P11) and tighten the Studio/Http boundary described above: relocate any Http-specific code that ended up in a shared or Studio-owned location into its own module directory, split any test file that grew to cover both modules' cases into one file per module (folder-separated, even where the runner command still runs both suites together), and audit `internal/bridge`, `frontend/src/state`, and `packages/shared` for coupling that would block splitting Http into a standalone app later. No new user-facing behavior — a structural pass, not a feature phase | Named up front as the phase every other phase should be minimizing work for, not a surprise cleanup at the end: doing this once, deliberately, after the module is feature-complete, is cheaper than either enforcing perfect separation through ten prior phases or leaving the coupling in place indefinitely |
| **P13 Http module code review (2 rounds)** | Two rounds of dedicated code review scoped to the Http module only — everything under Http's now-separated directories from P12, plus the Http-facing halves of `internal/httpclient`, `internal/postman`, and `internal/bridge` — the same discipline v1.1's P12 applied chapter-wide (`docs/v1.1/SPEC.md`), focused here on this chapter's own module | Sequenced last, after modularization, so the review reads the module in its final, separated shape rather than reviewing code P12 is about to move out from under it |
