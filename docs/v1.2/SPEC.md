...
| **P15 (new) Request builder input/editor UX pass** | A user-driven follow-up polish pass over the request builder's own surfaces and inputs: the response panel present from the moment a request tab opens rather than appearing only after a first send; a status code's meaning shown without requiring a hover; the Studio/Api mode-switch buttons sized up; the headers/params/form-data tables stretching to fill their available width; a new shared Checkbox primitive matching this app's own design system, replacing every raw `<input type="checkbox">` app-wide (Studio included, since primitives already live in the shared theme layer — `AppButton`/`TextField`/`SegmentedControl` are precedent); JSON promoted to its own top-level body mode out of the generic "Code" mode, given how dominant its real-world use is; header-name autocomplete over the common well-known HTTP header vocabulary; a body-present indicator on a request's tab, mirroring how a tab already flags its other state; the toolbar reflowed so Save sits with the request's name and the URL field gets much more of the available width; `{{variable}}` references coloured distinctly for resolved vs. unresolved, with a hover tooltip showing the resolved value and autocomplete over known variable names; auto-closing bracket/quote pairs in the URL/header/body inputs; and arrow-key navigation across these table/row-based inputs | Sequenced after P12/P13 so it refines a surface that has already settled into its final module shape and design-system pass, rather than polishing files about to be renamed or restyled out from under it — the same role P13 played, but driven by concrete user-reported friction rather than a self-directed audit. The `{{variable}}` colouring/hover/autocomplete and the editor-behaviour items (auto-closing pairs, arrow navigation) are the same family of investment as this app's existing SQL-console language service (v1.1 P18) and the `AutocompleteField.vue` overlay trick already used for the WHERE/ORDER BY fields — this phase's own plan decides whether that mechanism extends here or a different one is warranted, and whether the full list lands as one phase or is split into a follow-up, given its range from small layout fixes to real editor-behaviour work |
