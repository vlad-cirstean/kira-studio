...
**Numbering: C1-C8, restarting with a fresh prefix.** v1.3 used `G…` for the one reason it stated
...
**Eight rows is where this chapter opens, not where it ends.** The table accrues rows as phases
...
- **`packages/git-ui`** is the graph's whole Vue frontend — `CommitGrid.vue`, `graph/rowSvg.ts` +
  ...
  explicitly **out of scope for this chapter's native port** (user instruction) — every other
  graph/branch/stash/worktree/search feature in this package is in scope.
