...
**Numbering: C1-C14, restarting with a fresh prefix.** v1.3 used `G…` for the one reason it stated
...
**Fourteen rows is where this chapter stands today, not where it ends.** The table accrues rows as
...
own scope shrank; the split moved its diff-tab and code-navigation half into its own row. **C8 is a
second such renumbering, of a different shape**: not a split of an oversized phase, but a new phase
inserted mid-chapter once C7's own A/B-test dogfooding surfaced a real, closeable gap in C3's
server (see C8's own row) — worth fixing and re-measuring before the rest of the chapter proceeds,
per explicit instruction, so it landed here rather than waiting for a natural end-of-chapter slot.
Every phase from the old C8 (quick open) onward moved up by one to make room.
...
agent and once without, on independent worktrees) well before the editor-UI phases (C5 onward) exist
at all.
...
- **`packages/git-ui`** is the graph's whole Vue frontend — `CommitGrid.vue`, `graph/rowSvg.ts` +
  ...
  **deferred to the last feature phase of this chapter (C11)**, not ported alongside the rest of the
  graph in C10 — every other graph/branch/stash/worktree/search feature in `git-ui` lands with C10.
...
| **C3 MCP repository-map server** | A local MCP server exposing C2's code graph (definitions, references, implementations, symbol/file search) as tools an AI client calls instead of reading whole files — the token-reduction goal stated in the request. **Disabled by default.** A settings surface: an enable toggle that, once on, shows the exact CLI command the user runs to register this server with the Claude Code CLI *before* an Install button next to it — command visible first, button second, so enabling is never a silent action — and the same treatment (command shown, then an install action) for registering it as a VS Code extension's MCP server. **Runs headless via `bun run`** — a script in this repo's own `package.json`, matching every other dev/build/test entry point (see Grounding) — and is written so running it during this repo's own development never collides with the dev server, the git socket, or any port/file lock `wails3 task dev` already holds; this phase's own planning pass states the concrete isolation mechanism (a distinct port/socket, a lock file, or a design that needs neither) rather than asserting non-interference without one. Reads the same SQLite cache C1/C2 populate — no second parse pipeline | Moved up from after the editor-UI phases once it was clear it depends only on C2, never on Monaco/search/quick-open/the native git graph (C5 onward) — landing it here lets its actual value (does it cut token usage on a real task?) get measured right after C2, instead of waiting on five unrelated phases first |
