...
| **G21** | Code review, round 1 of 3 — a broad-dimension pass over everything this chapter added on top of v1.2 (all of `internal/git*`, `internal/bridge/rpcstream`, the migrated `apps/kira-studio-vscode` extension, `packages/git-*` wiring, and every design decision recorded in this file): correctness against each phase's own plan, security (fail-closed askpass per D8, argv-only spawning with no shell interpolation anywhere in `gitclient`, socket/pairing trust boundaries from G1), the layering rule (`internal` package test — git packages never import or get imported by `studio`/`api` adapters outside `bridge/rpcstream`), performance against the perf budgets re-baselined in G8, and drift between what a phase's `docs/v1.3/plans/G<N>-*.md` said and what actually shipped. Findings are recorded, not fixed — this round is discovery only, same "review then verify" split the codebase already uses for its own findings tooling | G1–G20 | new |
| **G22** | Code review, round 2 of 3 — adversarial verification of every round-1 finding: each one is confirmed with a concrete reproduction or dismissed with a specific counter-argument, never just re-read and taken on faith. Also its own second, independent dimension pass: test coverage gaps (especially the concurrency/teardown paths G8 exists to stress), error-message and UX text fidelity against upstream's byte-identical-label requirements (F9's undo labels, G16's per-commit indicator distinguishing "PR" questions), and whether any of the phases explicitly deferred to "this phase's own planning pass" (G16 onward) actually resolved the open question it deferred, or just moved the placeholder. Confirmed findings are fixed and pushed as part of this round, not deferred to round 3 | G21 | new |
| **G23** | Code review, round 3 of 3 — final pass, after rounds 1 and 2's fixes have landed: re-verify each fix didn't regress anything else it touched (a fix in one git package reaching into another is exactly what the layering test exists to catch), run the full unscoped `internal/` Go tree once under `-race` (per the "Full verification scope" note above — the occasional backstop this chapter otherwise skips per-phase), and a last holistic read of this file against the shipped code for anything `SPEC.md` still claims that the code doesn't do, or the reverse. This is the close-out gate for the whole v1.3 chapter, not a phase with its own feature | G22 | new |
| **G24** | Docs update — the chapter's actual closeout: fold everything the three review rounds (G21–G23) surfaced and fixed into the durable docs, not just this phasing table. `docs/ARCHITECTURE.md` gets the git module described as a real, shipped subsystem (transport, session model, the package list) rather than a plan; `AGENTS.md` gets any new environment/convention notes the reviews produced (in the same spirit as the verification-scope and `docs/pending-changes/` notes already added mid-chapter); each phase's own `docs/v1.3/plans/G<N>-*.md` gets a short note where the shipped code ended up diverging from the plan, rather than leaving the plan as the only record and the code silently disagreeing with it; and this file's own "Known open items" / "Out of scope" sections get swept for anything G1–G23 actually closed. No new code — this phase's only output is documentation, and it's the true last phase of v1.3, after which the chapter is done | G21, G22, G23 | new |
...
**Code review rounds, 2026-09-07.** G21–G23 close out the chapter with three rounds of code review
over everything v1.3 added on top of v1.2 — discovery (G21), adversarial verification plus fixes
(G22), and a final regression/consistency pass (G23) — rather than trusting each phase's own
per-phase verification (scoped to that phase's packages, per the "Full verification scope" note
above) to have caught everything across twenty-plus phases written by that many separate
implementing subagents. No upstream equivalent; added on request, sequenced last because every
other phase has to exist first for the review to be complete.
