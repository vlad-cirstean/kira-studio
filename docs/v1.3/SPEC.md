...
| `packages/git-ipc` | the source's `packages/ipc` | Git's own transport contract: the request/event/stream type map (`contract.ts`) and the `Transport` interface every host implements, instantiated over `packages/ipc-core`'s generic RPC endpoint bound to Git's version and method vocabulary (`endpoint.ts`/`validate.ts`) |
| `packages/ipc-core` | — (v1.3 P1 iter2) | The module-agnostic half of a correlated-RPC-with-credits protocol: the codec (including `ArrayBuffer` transfer lists), the versioned envelope, the contract-shape assertion, and the one generic RPC endpoint carrying correlation, stream credits and cancellation — the contract a type parameter, the version/vocabulary an injected config. Depends on nothing; a second module wanting the same request/response-plus-streaming shape (P12, possibly) depends on this rather than rebuilding it |
...
**Monaco, specifically, is scoped to Git's diff view (P4) and stays out of every other editor
surface.** `@codemirror/merge` (the CodeMirror team's own split/unified diff package, MIT, built on
the same `EditorState`/`EditorView` this app's `CodeMirrorHost.vue` already hosts) could render P4's
diff with no new dependency at all — that option was evaluated and is the lighter one, but Monaco is
the deliberate choice for Git's diff pane regardless. That does not reopen the question of a wider
CodeMirror→Monaco swap across the rest of the app (the query console, cell editor, document/DDL
views, and Http/Api's own request/response editors): those surfaces are built directly on
CodeMirror's Lezer-based language-service extension points (`autocompletion()`, `linter()`,
`hoverTooltip()`, the SQL/Mongo grammars in `editor/languages.ts` and `views/console/*`), which
Monaco does not share an API with — moving them would mean rebuilding that whole layer against
Monaco's own tokenizer/provider model, not reusing it. `AutocompleteField.vue`'s WHERE/ORDER BY
overlay trick is a second, independent reason those surfaces stay on CodeMirror: it keeps a real
native `<input>` as the interactive element specifically so Playwright's `.fill()` keeps working,
and Monaco's own hidden `<textarea>` is not a `.fill()`-able value-holder the way a native input is
— adopting Monaco there would not remove that constraint, only relocate it. So: two editor
libraries in the app is an accepted, explicit cost of Git's diff view — not the start of a broader
migration.
