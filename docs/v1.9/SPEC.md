...
| **P120 Audit and remove leftover git-related surface from Kira Studio** | User-reported: Kira Studio (a database/API client, not a git client) still has git-related material somewhere beyond its Settings page — the user has not named the exact spot, only that it's real and not limited to Settings. Both apps trace back to a shared app base (P100's per-app split, P103's shared `packages/workbench`/`internal/shell`), so the likely source is code that predates or survived that split rather than anything deliberately added to Studio. The planning pass must build a real inventory, not guess from Settings alone: `codegraph_explore`/grep Studio's own Go (`apps/kira-studio/internal/**`) and frontend (`apps/kira-studio/frontend/src/**`) trees for `git`-named packages, types, IPC bindings, menu items, views, or copy, cross-checking each hit against whether it's genuinely git-specific (a leftover) or a coincidental name (e.g. a `git` substring inside an unrelated word) or legitimately shared infrastructure Studio has a real reason to keep (e.g. this repo's own git-based version-control tooling, if any Studio-side code genuinely needs it). Acceptance: a committed inventory (file:line, verdict, disposition) plus every real leftover removed and both apps still building/typechecking/passing their existing suites | User-reported directly, appended as its own phase per CLAUDE.md's standing exception for a cross-cutting gap discovered outside any single in-flight phase's scope. Per CLAUDE.md's numbering rule the next sequential `P` number, appended after P119 |
| **P121 Tab-strip elements: sizing, corners and spacing inconsistent with the rest of the UI, both apps** | User-reported: the tab-strip UI pattern — named examples are Kira Studio's SQL/page tabs and Kira Space's repo/file-switcher tabs — doesn't fit the sizing/spacing of surrounding elements and has no rounded corners where the rest of the UI does. Likely a P110 Tailwind-migration-era gap (same class as P117's own visual-misalignment findings) rather than a new regression. The planning pass must actually run both apps (the `run` skill or a real dev build), screenshot every tab-strip instance in both apps side by side with a non-tab control that looks correct (buttons, other panel chrome), and name the exact Tailwind classes/tokens responsible for the mismatch before proposing a fix — not assume one tab component covers every instance, since Studio and Space may each have their own copy post-P100's split. Acceptance: every tab-strip instance in both apps matches the surrounding UI's corner radius and spacing scale, verified visually (screenshot or `test:visual` if a relevant spec already exists) | User-reported directly, appended as its own phase per CLAUDE.md's standing exception. Real file-overlap risk with P122/P123 (all three likely touch shared theme tokens/`packages/workbench` or `packages/theme`) — the orchestrating session sequences these three phases with that overlap in mind rather than treating CLAUDE.md's parallel-stream allowance as license to run them concurrently against each other. Per CLAUDE.md's numbering rule, appended after P120 |
| **P122 Focus-ring border too thick on every element that shows one, both apps** | User-reported: a focused input's border turns blue correctly, but far too thick; it should match the same thin ring already used correctly somewhere in the UI — the user names "the filter input in sql view" as the reference to match, without knowing its exact CSS value. The planning pass must locate that reference element, read its actual computed/declared value (a Tailwind `ring`/`outline`/`border` utility or a `--kira-*`/`--kui-*` token, per `docs/DEV_ENVIRONMENT.md`'s token setup), then audit every other focusable element in both apps for the same over-thick highlight (not just inputs — any focusable/clickable element showing this ring: buttons, selects, comboboxes, editable cells, etc.) and unify them to the one correct value, via the underlying shared token/utility rather than a one-off override per component. Acceptance: the SQL-view filter input's own value is unchanged and named explicitly as the source of truth; every other over-thick focus ring in both apps now renders at that same value; no component is fixed with a local override that bypasses the shared token | User-reported directly, appended as its own phase per CLAUDE.md's standing exception. Same overlap note as P121 — sequenced with P121/P123 rather than run concurrently against either. Per CLAUDE.md's numbering rule, appended after P121 |
| **P123 Font sizes inconsistent across both apps — normalize UI chrome to a small fixed set of sizes** | User-reported: font sizes vary by location almost arbitrarily across both apps. Scope is UI chrome only (labels, buttons, menus, panel headers, status bar, dialogs, tree/list rows) — the data-view surfaces (SQL results grid, document/JSON viewers, diff/blame views) stay user-customizable exactly as today, explicitly out of scope. The planning pass must first build a real inventory (grep every hardcoded font-size value and every existing `--kira-*`/`--kui-*` typography token/Tailwind text-size class actually in use across both apps' non-data-view surfaces) before proposing anything, then design a small fixed scale — the user suggests roughly 3-4 values — with a stated, sensible rationale for what each size is for (e.g. body/label default, secondary/caption, section heading, and at most one more), and migrate every UI-chrome surface onto exactly those values with zero ad-hoc exceptions. Acceptance: a committed inventory of every distinct font-size value found pre-fix; a stated 3-4-value scale with named purpose per value; every non-data-view surface in both apps uses only those values (grep-verifiable); data-view surfaces are provably untouched | User-reported directly, appended as its own phase per CLAUDE.md's standing exception. Same overlap note as P121/P122 — this is the most cross-cutting of the three and should very likely run after, not concurrently with, whichever of P121/P122 the orchestrating session runs first, since a token/scale change here plausibly touches the same files either already touched. Per CLAUDE.md's numbering rule, appended after P122 |
| **P124 Kira Space commit-details "Show more" button non-functional** | User-reported: in Kira Space's commit-details view, clicking "Show more" does nothing observable. The planning pass is explicitly a reproduce-first investigation, not a guess-and-patch: exercise the button under different scenarios (short vs. long commit body, different file-change counts, different selection/scroll states, keyboard vs. mouse activation) in a real running instance until the failure reproduces, trace the actual click handler/state update with `codegraph_explore` to find why it's a no-op (a dead binding, a state flag that never flips, a condition that's always false, an event that never fires), then fix the real cause. Acceptance: the reproduction steps and root cause are recorded, the fix is verified by re-running those same steps and seeing the expected expand behavior, and a regression test/UI-spec case is added if the bug shape makes one cheap and durable (per CLAUDE.md's own testing bar — not required if the fix is a one-line condition with no meaningful edge case to pin) | User-reported directly, appended as its own phase per CLAUDE.md's standing exception. Narrow, self-contained bug fix with essentially no file overlap with P120-P123 — a natural pairing partner for whichever of those runs concurrently with it. Per CLAUDE.md's numbering rule, appended last, after P123 |
| **P125 Rewrite root `README.md` as user-facing entry point: features, status, install** | User-reported: root README reads as internal engineering reference — full engine capability matrix with driver footnotes, a per-script dev table, a chapter-by-chapter v1-v1.9 history in both Status and Documentation. Rewrite so a reader quickly learns what Kira Studio is, what it does, its current state and how to install/run it; point to `apps/kira-space/README.md` for the sibling app. Deep reference (full per-engine matrix, every script, chapter history) links out to `docs/ARCHITECTURE.md`, `docs/DEV_ENVIRONMENT.md`, `docs/PACKAGING.md`, `docs/v1.9/`, not duplicated. Every retained claim checked against current source, not copied. README stays normal prose (`CLAUDE.md`'s exemption). Acceptance: README restructured and shortened; no feature, status or version claim without a source; diff touches only `README.md`, this row/result and the plan | User-reported directly, appended as its own phase per CLAUDE.md's standing exception. Docs-only, no file overlap with P121/P123. User authorized plan and rewrite in one Opus pass for this phase only. Per CLAUDE.md's numbering rule, appended last, after P124 |
