...
| **P119 Kira Space release feed and update notifier** | Blocked-item follow-up from P116's own inventory (B1, row 39): Kira Space has no update-available status-bar notifier at all, unlike Kira Studio's own `UpdateService`/`useAppUpdateStore`. Wiring it now would ship dead code — `.github/workflows/release.yml` builds, stamps and publishes only Studio's DMG on `v*.*.*` tags, `KS/build/config.yml`'s version stays the `"0.0.0"` dev sentinel so `appupdate.isReleaseBuild` is always false, and `appupdate.Checker` hard-codes Studio's own `releases/latest` even for a stamped build. Needs, in order: a Space release job (via `docs/pending-changes/`, same `.github/workflows/` push-scope workaround P116 itself used), a per-app tag scheme so one repo's releases page can host both apps' tags without colliding, `internal/appupdate` hoisted from Studio-only to repo-root and parameterized by repo/tag prefix (the same shared-primitive shape P116's own H1-H7 hoists used), then a `UpdateService` plus the status-bar item wired into Space through P116's shared bridge/store/component seams (`createCoreControl`, a `createAppUpdateStore` factory, the shared title/status-bar components). Acceptance: a stamped Kira Space build whose own tag is behind the latest Space release shows the same "Update available" status-bar item Studio shows, opens Space's own release page (never Studio's), and a dev build (`config.yml`'s `"0.0.0"` sentinel) shows nothing, exactly as Studio's own dev builds do today | P116's own §3 B1 — real gap, explicitly out of that phase's scope per its own acceptance criterion 3 and CLAUDE.md's out-of-scope-finding rule (a real design decision: the tag scheme, the release job shape) rather than a same-pass fix. Per CLAUDE.md's numbering rule the next sequential `P` number, appended last, after P118 |
