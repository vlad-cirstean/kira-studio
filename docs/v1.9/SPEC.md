...
| **P103 Normalize spacing/sizing onto Tailwind's own default scale** | `theme/base.css`'s `@theme` block only maps `--color-*` and `--radius-kira-*` into Tailwind v4's token system — spacing/sizing was never mapped. P99 preserved every pre-existing `--kira-s-*`/`--kira-control-*` value verbatim via Tailwind's arbitrary-value bracket syntax (measured: 425 `[...var(--kira-*)...]` usages across 115 `.vue` files), by that phase's own deliberate pixel-identical `test:visual` gate. This phase does the opposite on purpose: extend `@theme`'s spacing/sizing tokens to a coherent scale, replace arbitrary-bracket spacing/sizing utilities with Tailwind's own scale utilities app-wide, and let whatever alignment/spacing inconsistency that consolidation surfaces or fixes show up in the rendered app. Visual output is expected to change — new `test:visual` baselines get recorded per changed surface, with the reason stated per baseline, not treated as a diff to chase back to zero | User request, once P99 was found to have converted CSS syntax without normalizing the underlying spacing values it referenced — this phase is the actual normalization pass P99 was never scoped to do |
| **P104 Delete every hand-rolled UI primitive; call shadcn-vue's own components directly** | P99 wired `reka-ui`'s headless primitives into the app's existing hand-rolled `theme/primitives/*.vue` wrappers (keeping each wrapper's own prop/emit/slot API), and separately fetched the shadcn-vue-generated component files into `components/ui/` (18 components: `button`, `dialog`, `checkbox`, `dropdown-menu`, `popover`, `tooltip`, `command`, `context-menu`, and more) — those generated files were never imported by any app-level file; `theme/primitives/*` stayed the real call surface, leaving the fetched components as unused scaffolding. This phase deletes `theme/primitives/*.vue` (and any other hand-rolled primitive layer still standing) entirely and repoints every call site directly at `components/ui/*`'s shadcn-vue components — no wrapper layer, nothing fetched-but-unused left behind. Explicitly re-opens the three cases P99 declined with a named reason: `Checkbox` (`CheckboxRoot` renders `<button role="checkbox">`, not a real `<input>`, breaking Playwright `.check()` calls and native form semantics), `workbench/ContextMenu.vue` (reka-ui's trigger-anchored model vs. this app's point-anchored singleton), and `AppTooltip.vue` (directive-plus-singleton architecture). This time each must be genuinely resolved — updating the ~331 class-based Playwright selectors and any native-semantics reliance the swap breaks — and only kept hand-rolled if a requirement truly cannot be met even after that rework, stated explicitly in the result section, not silently stranded | User request, direct follow-up to this chapter's own finding that P99's "a component primitive comes from shadcn-vue" rule was satisfied only at the underlying `reka-ui` level, never at the component layer the app actually calls |
