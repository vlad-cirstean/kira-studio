...
- `@faker-js/faker` is already a dependency, used today only from the frontend's own "Generate fake
  data" grid feature. **M4 (a Go-to-Faker MCP bridge) was planned and partly built, then descoped**
  once M5's actual scope was clarified: M5 uses masking only, not realistic-value replacement, so
  nothing in this chapter needs a Faker bridge. See M4's own row for the full disposition.
...
before EXPLAIN-gating logic is added on top of it. **M4 (Faker over MCP) was planned and partly
built, then descoped** — M5's actual requirement, clarified in conversation after M4 landed its
plan and its bridge commits, is masking only: real values are never replaced with realistic-looking
fake ones, only partially redacted, so M4's whole reason to exist (a deterministic realistic-value
generator) has no consumer in this chapter. M5 now comes right after M3, needing only M1's query
path and M2's connection model, both already landed. M6/M7 (the two
...
| **M4 Faker exposed via MCP** — **descoped, not built** | Planned in full (`docs/v1.7/plans/M4-faker-bridge.md`, kept as the historical record of the design work, not implemented): a `generate_fake_data` tool bridging the Go MCP server to `@faker-js/faker` via an embedded `goja` JS VM (measured byte-identical to real Bun/faker across all 24 generators before being chosen over a subprocess, which would silently not work in a shipped binary with no JS runtime). Four commits landed (`55229696`..`ca8b2e02`, the goja bridge itself) then discarded by explicit instruction once M5's actual requirements were clarified in conversation: M5 does not want realistic fake-value replacement at all, only masking — so M4's entire reason to exist (a deterministic *realistic-looking* value generator) no longer has a consumer in this chapter. Nothing from M4 ships | Superseded: M5's own scope, clarified after M4 was planned and partly built, doesn't use it |
| **M5 Anonymization filters: per-column PII rules, format-preserving masking, applied over MCP and in the normal data viewer** | Per-column filter rules (which table.column pairs are PII) configurable per connection — this phase's own planning pass decides the concrete UI (likely a table within M2's own MCP tab) and the storage shape (a new table/migration). **Masking only, by explicit instruction** — no Faker-based realistic-value replacement (M4 descoped for exactly this reason), and hashing alone was explicitly ruled out: a low-entropy value like a first name is guessable even hashed-and-salted (a dictionary/preimage attack against a small candidate space, not a hash-strength problem), so masking — genuinely deleting characters rather than transforming them — is the mechanism. Deterministic per real value (same input always produces the same masked output within a connection), so correlation and referential joins across a foreign key survive without ever exposing or being able to recover the real value. This phase's own planning pass designs the actual masking scheme(s) per PII class (a name vs. an email vs. a numeric/date field each need a different masking shape) and states why. Applied in two places: (1) MCP query results pass through the configured filter before being returned to an AI client, per column; (2) the normal data viewer — `views/grid/DataView.vue`'s grid and the `celleditor/` cell editor — gets a toggle to view the same masked transform live over real data, per explicit instruction ("show using the normal data viewer"), never mutating the underlying row, purely a display-layer transform. **The plan's own `id` kind (full redact + tag, for a marked-sensitive identifier-shaped column) was dropped mid-implementation, by explicit instruction** — masking is opt-in per column, so an internal identifier nobody marks sensitive is already untouched; five other kinds (name/email/text/number/date) plus `redact` ship | Right after M3: no longer depends on M4 (descoped); needs M2's connection model and M1's query/result path, both already landed |
**M5 result**: landed per plan (`docs/v1.7/plans/M5-anonymization-masking.md`), 6 commits
(`f22cc301`..`4aba9458`), migration `0023_m5_column_mask_rules.sql`. **One scope change mid-
implementation, by explicit instruction: the plan's `id` masking kind was dropped entirely** —
masking is opt-in per column, so an internal identifier never marked sensitive is untouched by this
system already; the plan's own `id` kind (full redact + tag, for a marked-sensitive identifier-shaped
column) was judged unneeded. Six kinds ship: name/email/text/number/date/redact. `internal/mask`
(grapheme-aware via `uniseg`, keyed HMAC-SHA256 correlation tag, `Stricter()` conflict folding) is
ported to TypeScript (`packages/shared/domain/mask.ts`), pinned against 60 shared Go-generated
fixture pairs under `tests/fixtures/mask/` both `mask_test.go` and `mask-parity.spec.ts` read.
Applied at the MCP render path (`internal/dbmcp/render.go`, a 4th `renderPage` param; document/
stream pages refuse rather than silently skip when rules exist) and in the grid (`maskPreview.ts`, a
display-only transform that never mutates `SlickGrid`'s cached view; edit lockout cascades from
`canEditTable`/`canDeleteRows`/toolbar `isWritable` all gating on `!maskPreviewOn()`). Independently
re-verified: `go build/vet` clean, `internal/mask` 62/62 under `-race` (incl. a 500-iteration
never-echoes-original-text property test), `bun typecheck`/`lint` clean, `bun run test:unit` 1431/0,
full `tests/ui/` suite 274/274. Empirically checked against a real running app (edit lockout via
Playwright against a real server+adapter+browser) and a real `dbmcp` HTTP endpoint via curl (masked
`run_query` output showed same value → same tag for two "Maria Gonzalez" rows, different tag for
"Kenji Tanaka" — determinism and non-collision both confirmed live, not just unit-tested). One
leftover found on independent review — two comments in `mask.go`/migration `0023` still listed the
