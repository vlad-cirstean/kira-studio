...
- Autocompact is configured — do not stop between steps to `/compact`.
...
- **No pre-connect scripts.** §1 defers them to P12 explicitly — no field in the dialog, no column use, no shell execution anywhere.
...
| **Catalog queries slow on a large database** | Expanding a schema with 40k relations takes seconds | Acceptable in P1 *provided* it is cancellable (D5) and shows a spinner. Do not add pagination to the tree — that is a real design question and belongs to P11's hardening pass with measurements behind it. |
