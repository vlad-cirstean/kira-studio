...
| Text editing / viewing | `monaco-editor` (every studio/api editor surface but the SQL console/DDL editor, P60a) + CodeMirror 6 (the SQL query console, `project/SchemaDialog.vue`'s DDL editor — P60b's own scope) | P60a migrated the 15 other CodeMirror mount sites (`editor/MonacoHost.vue`, a drop-in for the deleted-in-P60b `editor/CodeMirrorHost.vue`: same props/emits/exposed methods) plus `AutocompleteField.vue`'s filter-field overlay (span-painted text via `monaco.editor.colorize()`+`tokenize()`, not a mounted editor — `editor/paintSpans.ts`) and the response-history diff dialog (Monaco's own diff editor, replacing `@codemirror/merge`, now removed from `package.json`). Every editor surface — read-only viewer and real editable host alike — now loads Monaco through the one shared, memoised `loadMonaco()` (`editor/monaco.ts`); nothing is eager, so a studio/api session that opens no editor fetches neither engine and a session that opens one pays the ~972 KB gzip chunk once, cached thereafter. The SQL console and DDL editor stay on CodeMirror only because they need `@codemirror/lang-sql`'s Lezer parse tree for completion/hover/diagnostics/DDL extraction — a genuinely different requirement (P60b's plan), not an engine preference; `docs/v1.6/plans/P60a-monaco-host-and-general-surfaces.md` has the full capability-by-capability mapping (theme/token colours, range highlights, hover, completion, lint, the undo boundary around an external write, wrap-on-type, auto-close brackets) |
