...
| Data grid rendering (P22 spike, Pass A only) | `slickgrid@5.20.0`'s core engine (MIT, `6pac/SlickGrid`), core `SlickGrid` class only — no `SlickDataView`, no plugin, no `slickgrid-vue` | a from-scratch Vue host (`views/grid/SlickGridHost.vue`, on `editor/CodeMirrorHost.vue`'s own imperative-library pattern), reachable behind `window.__kiraGridEngine === 'slick'` — the production grid stays `views/grid/DataGrid.vue`/`@tanstack/vue-virtual` until a real-hardware A/B authorises a cutover (`docs/v1.1/plans/P22-slickgrid-migration-plan.md` §7). Measured import cost: `{ SlickGrid }` alone is 41 948 B gzip (esbuild@0.28.2, this session — the plan's own F9 projected 41 947 B); `SlickDataView` and every plugin (context menu, range selection, clipboard) are deliberately not imported — see that plan's F9/§4 for why each was declined |
