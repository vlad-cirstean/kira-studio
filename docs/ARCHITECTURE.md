...
| Lint + format | Biome (TS/Vue, incl. its `performance` rule group as of P94), `golangci-lint` (Go — `bodyclose`, `copyloopvar`, `gocritic`'s performance tag, `govet`, `ineffassign`, `makezero`, `prealloc`, `unconvert`, `unused`), `knip` (TS/Vue dead files/dependencies/duplicate exports) | Biome for TS/Vue formatting stays the single tool there, no ESLint/Prettier; the other two are pre-push-hook/CI-only (`.githooks/pre-push`, `docs/DEV_ENVIRONMENT.md`), not per-commit |
