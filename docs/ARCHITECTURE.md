...
`internal/metrics`, `internal/localauth`, and — as of v1.3's G9, now Kira Space's own
`internal/gitclient` — its FSEvents-backed repo watcher)
...
either source re-asserts a level it already holds). While the set is non-empty, macOS runs
...
registering the listener is what makes Wails post `events.Mac.ApplicationDidWake` at all — re-arms
the assertion (release, then re-acquire) on every resume, a no-op while nothing is held. Every
non-darwin build gets a documented no-op driver instead: the titlebar hides its button outright
rather than offering a control that does nothing, and the agent-aware Settings leaf still persists
but never spawns anything.
