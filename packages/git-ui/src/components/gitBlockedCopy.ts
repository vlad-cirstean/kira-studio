 ...
 *
 * The block state names "the platform's upgrade command" as a single line, but `GitStatus`
 * carries no platform field (`git-core`/`git-ipc` never learn the *host OS*, only the host *kind*
 * — kira-studio/harness, see `HostKind`), and this is deliberately UI copy rather than wire data.
 * The
      ...
      return {
        ...
        detail:
          status.probed.length > 0
            ? `Looked for git at: ${status.probed.join(', ')}. Install git, or set git.path to point at it.`
            : 'Install git, or set git.path to point at it.',
      };
    case 'tooOld':
      return {
        title: 'Git is too old',
        detail: `Found ${status.detected} at ${status.path}; Git mode needs at least ${status.required}. Run "${upgradeCommandFor(platform)}", or set ${status.settingId} to a newer git.`,
      };
    case 'unusable':
      return {
