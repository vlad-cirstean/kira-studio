 ...
 *
 * **The fallback, and what has to be re-confirmed here.** The source project verified a module
 * worker loads under a VS Code webview's CSP `worker-src` directive. This app's webview is
 * Wails/WKWebView with its own asset server and its own CSP, so that confirmation does not carry
 * over and P1 has to re-establish it. If a module worker cannot load there, the fix is `submit`
 * running `layoutAppend` on the main thread instead, chunked so it never holds a frame — a real
 * degradation to record, not a reason to restructure anything above this file: every consumer is
 * already written against a promise. `createLayoutClient`'s `workerFactory` parameter is what
