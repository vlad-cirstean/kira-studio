...
// AttachCloseFlush's own `flushing` guard makes the hook one-shot per window. sig is the
// Signaller (P103 Part 3) AttachCloseFlush signals the renderer through — the coordinator's own
// job is routing the ack back, so it is the natural owner of the one Signaller it needs to ask
...
func NewCloseFlushCoordinator(sig Signaller) *CloseFlushCoordinator {
	return &CloseFlushCoordinator{sig: sig, waiting: map[string]chan struct{}{}}
}
...
func AttachCloseFlush(
	...
	win.RegisterHook(wailsevents.Common.WindowClosing, func(event *application.WindowEvent) {
		...
		coordinator.sig.SignalTo(key, appevent.ChannelWindowFlushBeforeClose)
		go func() {
			select {
			case <-ack:
