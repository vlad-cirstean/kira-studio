...
describe('postgres adapter (§9.1)', () => {
  ...
  test('31. count issues one statement (F13, D13)', async () => {
    const adapter = await createAdapter('postgres', deps);
    await adapter.connect(fixture.config, makeCtx());
    try {
      const ctx = makeCtx();
      ...
      // Was 4 before D13: getRelationInfo + listColumns + listIndexes + the count itself. Now
      // just the count — countRows never reads what those three catalog queries returned.
  ...
  test('32. read still resolves the catalog (D13 boundary — P2 D10 unchanged)', async () => {
    const adapter = await createAdapter('postgres', deps);
    await adapter.connect(fixture.config, makeCtx());
    try {
      const ctx = makeCtx();
      ...
      // D13 only touched count(); read()'s catalog resolution (relation info, columns, indexes)
      // plus the data query itself is untouched, so this still issues more than one statement.
  ...
  test('33. the running-query map does not grow (F3, D3)', async () => {
    const adapter = await createAdapter('postgres', deps);
    await adapter.connect(fixture.config, makeCtx());
    try {
      const target = path([
  ...
  test('34. a failed connect leaves nothing open (F1, D1)', async () => {
    const adapter = await createAdapter('postgres', deps);
    const badConfig = { ...fixture.config, password: 'definitely-wrong' };
