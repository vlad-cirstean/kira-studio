...
// Built by `bun run build:space` (vite.config.ts → apps/kira-space/frontend/dist), the exact
// bytes the real Wails bundle embeds (main.go's `//go:embed all:frontend/dist`) — this tier serves
// the same static output a packaged app does, just over plain HTTP instead of Wails' own
// AssetServer. Ported from Kira Studio's own tests/ui/support/server.ts — identical shape, this
// app's own relative depth from `tests/ui/support/` to `frontend/dist` happens to match exactly.
