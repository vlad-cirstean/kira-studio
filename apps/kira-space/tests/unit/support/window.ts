...
// P100 Part 2: this whole support/ directory is copied verbatim from apps/kira-studio/tests/unit/
// support/ — generic Wails-runtime-mocking test infra with zero per-app divergence (same
// "duplicate, don't hoist" call as editor/monaco.ts and views/terminal/TerminalView.vue's own doc
// comments; each app is a separate Vite/Bun project with no shared package boundary for src-level
// code). Keep the two copies in sync by hand if the mocking strategy itself ever changes.
//
...
// module scope (Stream('engine'), Events.On), so any spec whose chain reaches either now needs
