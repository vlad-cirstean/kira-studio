...
// every parseable source file has no use for a parseable-extension filter. Moved here from
// internal/codeindex (P97): ListFiles (files.go) and Search (search.go) are its only two callers
// left once P97 drops the parse/index/graph machinery that owned the rest of that package — the
// argv, the gitclient.Spec shape and the tier-2 path-byte rule (git's own bytes, never
// NFC-normalized) are unchanged.
