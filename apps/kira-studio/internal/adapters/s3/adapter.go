...
// KeyTypes — caps.KeyTypes is false; unreachable. An S3 object has no per-item engine-level
// "type" the way a redis key's TYPE command reports one (caps.go's own comment).
