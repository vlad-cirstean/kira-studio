...
var Provenance = []QuerySource{
	...
	// P67f-authored value-read query (docs/v1.6/plans/P67f-…md §3.3) — the same gap one language
	// family over. One file, registered directly on JavaScript, TypeScript and TSX alike (below):
	// its two patterns (for_in_statement/subscript_expression) are plain syntax every one of the
	// three grammars shares, unlike tags.scm's own languages which need composing through
	// javascript's own file first.
	{JavaScript, thisRepo, "queries/javascript/p67f_reads.scm", ""},
