...
// writes the repository (§4.1's table) and MUST be refused with E_READ_ONLY, and — the part a
// weaker test could miss — MUST NEVER reach the inner handler at all. A wrapper that returned
// E_READ_ONLY on some other code path while still calling next underneath (e.g. to log it, or by
// accident) would pass a test that only checks the returned error; the spy's own call count is
// what catches that.
func TestReadOnlyRequest_WriteMethodsAreRefused(t *testing.T) {
	writeMethods := []string{
		...
		"review.mark", "review.comment.add", "review.comment.remove", "review.comment.clear",
		"review.session.save",
	}
