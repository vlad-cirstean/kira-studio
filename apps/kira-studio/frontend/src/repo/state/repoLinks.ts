...
// an unknown id, via Map.get's own undefined) means top-level; a non-empty value names the
...
/** One batched `RepoWorktreeLinks` call — every row, no `ids` scoping (§4.2: both callers refresh
 *  the whole list). A row RepoWorktreeLinks could not read comes back with `parentId: ''` already
 *  (the Go side's own safe default, §4.2) — nothing extra to special-case here. */
 ...
 *  writes the answer switchToWorktree already knows, so the duplicate top-level row never flashes.
 *  The next batched call overwrites it either way; this is never the sole source of truth. */
export function noteWorktreeLink(childId: string, parentId: string): void {
