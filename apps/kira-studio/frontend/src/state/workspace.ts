...
export const useWorkspaceStore = defineStore('workspace', () => {
  ...
  // P67b §4.2: state.active is the single source of truth; modeState.active is its
  ...
  function activateWorkspace(key: WorkspaceKey): void {
    ...
    state.active = key;
    if (mode === 'git' && isRepoWorkspace(key)) state.lastRepoKey = key;
  }
  ...
  // module (the store) and its own doc comment already establishes that import direction as
  ...
  function closeRepoWorkspace(repoId: string): void {
    ...
    if (state.active === key) activateWorkspace('git');
  }
