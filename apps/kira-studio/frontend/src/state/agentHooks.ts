...
export const agentHooksState = reactive({
  status: DEFAULT_STATUS as AgentHooksStatus,
});

export async function hydrateAgentHooks(): Promise<void> {
  agentHooksState.status = await control.agentHooksStatus();
}
...
export async function setAgentHooksEnabled(enabled: boolean): Promise<void> {
  agentHooksState.status = await control.agentHooksSetEnabled(enabled);
