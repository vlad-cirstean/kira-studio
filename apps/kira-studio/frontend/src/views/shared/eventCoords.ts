...
// included. DataGrid.vue found and fixed this first (P2 R1): bind a stable, module-scope
// `on*FromEvent` function instead, and have it recover its row/column position from a data-*
// attribute on the element the listener actually fired on. This is that recovery step, factored
// out so ConsoleResultGrid/StreamView/KeyValueView don't each carry their own copy.
//
...
// whose row index actually lives on an ancestor row element, the same shape DataGrid's own
// onCellNavClickFromEvent needed for its nav button.
