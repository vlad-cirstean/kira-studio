// Real captured pageSize-100 read of a 100-row [id, md5(id)] table — byte-identical across three
// fixtures/specs (postgresFixture.ts's `BIG_ROWS_FIRST_PAGE`, data-view.spec.ts's `PAGE_C`, and
// tests/ipc/mariadb/mariadb.fixture.ts's `big_rows` response), each of which wraps it in its own
// engine-specific columns/position (dataType and column name differ mariadb vs postgres, and each
// capture's own real `nextToken` cursor differs). mariadb.fixture.ts itself does NOT import this —
// it is generated by internal/ipcfixture/mariadb_test.go (KIRA_IPC_FIXTURES=write) and marked
// do-not-hand-edit (P50 D5); only the two hand-written sites below import it.
export const MD5_ROWS: [id: string, hash: string][] = [
