...
`bun run build:studio` (see `frontend/vite.config.ts`) — but `frontend/src` itself imports the generated
...
bun run build:studio                   # builds frontend/src into apps/kira-studio/frontend/dist
cd apps/kira-studio
go run .
```
